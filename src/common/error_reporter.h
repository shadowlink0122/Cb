...
class ErrorReporter {
  public:
     ...
     */
    ErrorReporter(const std::string &src, const std::string &filename,
     ...
     */
    void report(ErrorSeverity severity, const SourceLocation &location,
     ...
     */
    void reportSpan(ErrorSeverity severity, const SourceSpan &span,
                    const std::string &message,
                    const std::vector<std::string> &suggestions = {}) const {
        ...
        if (span.isValid() && !source_.empty()) {
            if (span.isSingleLine()) {
                // Single line span
                std::string line =
                    SourceLocationUtils::extractLine(source_, span.start.line);
                ...
                for (const auto &[line_num, line_content] : context) {
                    if (line_num >= span.start.line &&
                        line_num <= span.end.line) {
                        oss << formatLineNumber(line_num) << " | "
                            << line_content << "\n";
                    }
                }
            }
     ...
     * @param message Error message
     */
    void reportSimple(ErrorSeverity severity,
                      const std::string &message) const {
        std::cerr << filename_ << ": " << severityToString(severity) << ": "
                  << message << "\n";
    }
    ...
    findSuggestions(const std::string &unknown_name,
                    const std::vector<std::string> &known_names,
                    size_t max_suggestions = 3) {

        auto similar =
            SourceLocationUtils::findSimilarStrings(unknown_name, known_names);
  ...
  private:
    ...
    [[maybe_unused]] bool
        use_colors_; ///< Enable color output (reserved for future use)
