...
HIRExpr HIRExprConverter::convert_expr(const ASTNode *node) {
    ...
    case ASTNodeType::AST_INTERPOLATED_STRING: {
        ...
        if (node->interpolation_segments.size() == 1) {
            ...
            return generator_->convert_expr(node->interpolation_segments[0].get());
        }

        // 複数セグメントの場合、順次連結
        auto result = generator_->convert_expr(node->interpolation_segments[0].get());
        for (size_t i = 1; i < node->interpolation_segments.size(); i++) {
            ...
            concat.right = std::make_unique<HIRExpr>(
                generator_->convert_expr(node->interpolation_segments[i].get()));
    ...
    case ASTNodeType::AST_BINARY_OP: {
        ...
        expr.op = node->op;
        expr.left = std::make_unique<HIRExpr>(generator_->convert_expr(node->left.get()));
        expr.right = std::make_unique<HIRExpr>(generator_->convert_expr(node->right.get()));
        break;
    }
    ...
    case ASTNodeType::AST_MEMBER_ACCESS: {
        ...
        expr.object = std::make_unique<HIRExpr>(generator_->convert_expr(node->left.get()));
    ...
    case ASTNodeType::AST_ARRAY_REF: {
        ...
        expr.array = std::make_unique<HIRExpr>(generator_->convert_expr(node->left.get()));
        expr.index =
            std::make_unique<HIRExpr>(generator_->convert_expr(node->array_index.get()));
        break;
    }
    ...
    case ASTNodeType::AST_CAST_EXPR: {
        ...
        if (node->cast_expr) {
            expr.cast_expr =
                std::make_unique<HIRExpr>(generator_->convert_expr(node->cast_expr.get()));
        } else if (debug_mode) {
        ...
        if (node->cast_type_info != TYPE_UNKNOWN) {
            expr.cast_type =
                generator_->convert_type(node->cast_type_info, node->cast_target_type);
        } else {
            expr.cast_type = generator_->convert_type(node->type_info, node->type_name);
        }
        break;
    }
    ...
    case ASTNodeType::AST_TERNARY_OP: {
        ...
        expr.else_expr =
            std::make_unique<HIRExpr>(generator_->convert_expr(node->third.get()));
        break;
    }
    ...
    case ASTNodeType::AST_RANGE_EXPR: {
        ...
        if (node->range_end) {
            expr.range_end =
                std::make_unique<HIRExpr>(generator_->convert_expr(node->range_end.get()));
        }
        break;
    }
    ...
    case ASTNodeType::AST_STRUCT_LITERAL: {
        ...
        for (const auto &arg : node->arguments) {
            if (arg->node_type == ASTNodeType::AST_ASSIGN) {
                ...
                if (arg->right) {
                    expr.field_values.push_back(generator_->convert_expr(arg->right.get()));
                }
            }
        }
        ...
        if (!has_named_init) {
            for (const auto &arg : node->arguments) {
                expr.field_values.push_back(generator_->convert_expr(arg.get()));
            }
        }
        break;
    }
    ...
    case ASTNodeType::AST_ARRAY_LITERAL: {
        ...
        for (const auto &element : node->arguments) {
            expr.array_elements.push_back(generator_->convert_expr(element.get()));
        }
        break;
    }
    ...
    case ASTNodeType::AST_NULLPTR: {
        ...
        //     expr.kind = HIRExpr::ExprKind::AddressOf;
        //     expr.operand =
        //     std::make_unique<HIRExpr>(generator_->convert_expr(node->left.get())); break;
        // }
    ...
    case ASTNodeType::AST_SIZEOF_EXPR: {
        ...
        if (node->left) {
            ...
            // the original name
            if (is_generic) {
                expr.sizeof_type = generator_->convert_type(TYPE_GENERIC, type_name_to_use);
            } else {
    ...
    case ASTNodeType::AST_LAMBDA_EXPR: {
        ...
        if (node->body) {
            expr.lambda_body =
                std::make_unique<HIRStmt>(generator_->convert_stmt(node->body.get()));
        }
        break;
    }
