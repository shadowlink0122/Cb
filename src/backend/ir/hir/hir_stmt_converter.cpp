...
HIRStmt HIRStmtConverter::convert_stmt(const ASTNode *node) {
        ...
        if (!node->children.empty()) {
            ...
            if (node->children.size() > 1) {
                ...
                for (const auto &var_node : node->children) {
                    ...
                    var_stmt.location = generator_->convert_location(var_node->location);
    ...
    case ASTNodeType::AST_ARRAY_DECL: {
        ...
        if (node->array_type_info.is_array()) {
            ...
            stmt.var_type = generator_->convert_type(node->type_info, node->type_name);
        }
    ...
    case ASTNodeType::AST_IF_STMT: {
        ...
        if (node->left) {
            stmt.then_body =
                std::make_unique<HIRStmt>(generator_->convert_stmt(node->left.get()));
        } else if (node->body) {
            // 後方互換性のためbodyもチェック
            stmt.then_body =
        ...
        if (node->right) {
            ...
            stmt.else_body =
                std::make_unique<HIRStmt>(generator_->convert_stmt(node->right.get()));
        } else if (node->else_body) {
            // 後方互換性のためelse_bodyもチェック
            stmt.else_body =
                std::make_unique<HIRStmt>(generator_->convert_stmt(node->else_body.get()));
        }
        break;
    }
    ...
    case ASTNodeType::AST_WHILE_STMT: {
        ...
        stmt.condition =
            std::make_unique<HIRExpr>(generator_->convert_expr(node->condition.get()));
        stmt.body = std::make_unique<HIRStmt>(generator_->convert_stmt(node->body.get()));
        break;
    }
    ...
    case ASTNodeType::AST_FOR_STMT: {
        ...
        if (node->init_expr) {
            stmt.init =
                std::make_unique<HIRStmt>(generator_->convert_stmt(node->init_expr.get()));
        }
        if (node->condition) {
            stmt.condition =
                std::make_unique<HIRExpr>(generator_->convert_expr(node->condition.get()));
    ...
    case ASTNodeType::AST_STMT_LIST: {
        ...
        for (const auto &child : node->statements) {
            if (child) {
                stmt.block_stmts.push_back(generator_->convert_stmt(child.get()));
            }
        }
    ...
    case ASTNodeType::AST_PRINT_STMT: {
        ...
        call_expr.func_name = (node->node_type == ASTNodeType::AST_PRINTLN_STMT)
                                  ? "println"
                                  : "print";
        call_expr.type = hir::HIRBuilder::make_basic_type(HIRType::TypeKind::Void);

        // 引数を変換（単一引数の場合はleft、複数引数の場合はarguments）
        if (node->left) {
            call_expr.arguments.push_back(generator_->convert_expr(node->left.get()));
        } else if (!node->arguments.empty()) {
            for (const auto &arg : node->arguments) {
                call_expr.arguments.push_back(generator_->convert_expr(arg.get()));
            }
        }
    ...
    case ASTNodeType::AST_SWITCH_STMT: {
        ...
        stmt.switch_expr =
            std::make_unique<HIRExpr>(generator_->convert_expr(node->switch_expr.get()));
            ...
            if (!case_node->case_values.empty()) {
                ...
                if (case_node->case_values[0]) {
                    hir_case.case_value = std::make_unique<HIRExpr>(
                        generator_->convert_expr(case_node->case_values[0].get()));
                }
                // case_bodyを設定
                ...
                for (size_t i = 1; i < case_node->case_values.size(); i++) {
                    if (case_node->case_values[i]) {
                        ...
                        fallthrough_case.case_value = std::make_unique<HIRExpr>(
                            generator_->convert_expr(case_node->case_values[i].get()));
                        // bodyは空（fall-through）
        ...
        if (node->else_body) {
            ...
            for (const auto &else_stmt : node->else_body->statements) {
                default_case.case_body.push_back(generator_->convert_stmt(else_stmt.get()));
            }
    ...
    case ASTNodeType::AST_TRY_STMT: {
        ...
        if (node->try_body) {
            for (const auto &try_stmt : node->try_body->statements) {
                stmt.try_block.push_back(generator_->convert_stmt(try_stmt.get()));
            }
        }
        ...
        if (node->finally_body) {
            for (const auto &finally_stmt : node->finally_body->statements) {
                stmt.finally_block.push_back(generator_->convert_stmt(finally_stmt.get()));
            }
        }
        break;
    }
