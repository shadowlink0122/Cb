...
std::string HIRToCpp::generate(const HIRProgram &program) {
    ...
    emit_line("");
    emit_line("    // Copy constructor");
    emit_line("    Option(const Option& other) : tag(other.tag) {");
    emit_line("        if (tag == Tag::Some) {");
    emit_line("            new (&some_value) T(other.some_value);");
    emit_line("        } else {");
    emit_line("            none_placeholder = other.none_placeholder;");
    emit_line("        }");
    emit_line("    }");
    emit_line("");
    emit_line("    // Copy assignment");
    emit_line("    Option& operator=(const Option& other) {");
    emit_line("        if (this != &other) {");
    emit_line("            if (tag == Tag::Some) {");
    emit_line("                some_value.~T();");
    emit_line("            }");
    emit_line("            tag = other.tag;");
    emit_line("            if (tag == Tag::Some) {");
    emit_line("                new (&some_value) T(other.some_value);");
    emit_line("            } else {");
    emit_line("                none_placeholder = other.none_placeholder;");
    ...
    emit_line("");
    emit_line("    // Copy constructor");
    emit_line("    Result(const Result& other) : tag(other.tag) {");
    emit_line("        if (tag == Tag::Ok) {");
    emit_line("            new (&ok_value) T(other.ok_value);");
    emit_line("        } else {");
    emit_line("            new (&err_value) E(other.err_value);");
    emit_line("        }");
    emit_line("    }");
    emit_line("");
    emit_line("    // Copy assignment");
    emit_line("    Result& operator=(const Result& other) {");
    emit_line("        if (this != &other) {");
    emit_line("            if (tag == Tag::Ok) {");
