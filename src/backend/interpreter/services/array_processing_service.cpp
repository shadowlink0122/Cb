...
ArrayProcessingService::ArrayProcessingService(Interpreter *interpreter,
                                               CommonOperations *common_ops)
    : interpreter_(interpreter), common_operations_(common_ops) {}

ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processArrayLiteral(const std::string &target_name,
    ...
    try {
        ...
        logArrayOperation(context,
                          "Processing array literal for: " + target_name);
        ...
        if (result.success) {
            logArrayOperation(context, "Successfully processed array for: " +
                                           target_name);
        }
        ...
        result.error_message =
            "Array processing error: " + std::string(e.what());
    }
...
ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processArrayDeclaration(const ASTNode *node,
                                                ArrayContext context) {
...
ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::assignArrayLiteral(Variable *target_var,
                                           const ASTNode *literal_node,
                                           ArrayContext context) {
    ...
    try {
        ...
        common_operations_->assign_array_literal_to_variable(
            target_var, array_result, resolved_name);
        ...
        result.success = false;
        result.error_message =
            "Array assignment error: " + std::string(e.what());
    }
...
int64_t
ArrayProcessingService::getArrayElement(const std::string &var_name,
    ...
    if (var->is_multidimensional) {
        ...
        return interpreter_->get_array_manager()
            ->getMultidimensionalArrayElement(*var, indices);
    } else {
        // 1次元配列の場合
        if (indices.size() != 1) {
...
void ArrayProcessingService::setArrayElement(
    const std::string &var_name, const std::vector<int64_t> &indices,
    int64_t value, ArrayContext context) {
    ...
    if (var->is_multidimensional) {
        // 多次元配列は直接ArrayManagerに委譲
        interpreter_->get_array_manager()->setMultidimensionalArrayElement(
            *var, indices, adjusted_value);
    } else {
        // 1次元配列の場合
        if (indices.size() != 1) {
            throw std::runtime_error("Invalid index count for 1D array");
...
std::string ArrayProcessingService::getStringArrayElement(
    ...
    if (var->is_multidimensional) {
        // 多次元配列は直接ArrayManagerに委譲
        return interpreter_->get_array_manager()
            ->getMultidimensionalStringArrayElement(*var, indices);
    } else {
        // 1次元配列の場合
        if (indices.size() != 1) {
            throw std::runtime_error("Invalid index count for 1D array");
...
void ArrayProcessingService::setStringArrayElement(
    const std::string &var_name, const std::vector<int64_t> &indices,
    const std::string &value, ArrayContext context) {
    ...
    if (var->is_const && var->is_assigned) {
        throw std::runtime_error("Cannot assign to const string array: " +
                                 var_name);
    }

    if (var->is_multidimensional) {
        // 多次元配列は直接ArrayManagerに委譲
        interpreter_->get_array_manager()
            ->setMultidimensionalStringArrayElement(*var, indices, value);
    } else {
        // 1次元配列の場合
        if (indices.size() != 1) {
            throw std::runtime_error("Invalid index count for 1D array");
...
TypeInfo
ArrayProcessingService::inferArrayElementType(const ASTNode *literal_node) {
...
bool ArrayProcessingService::validateArrayOperation(const Variable *var,
                                                    const ASTNode *node,
                                                    ArrayContext context) {
    return var && node; // 簡素化
}
...
// コンテキスト別処理実装
ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processVariableArray(const std::string &name,
                                             const ASTNode *literal_node) {
    ArrayOperationResult result = {};
...
ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processStructMemberArray(const std::string &name,
                                                 const ASTNode *literal_node) {
    return processVariableArray(name, literal_node); // 簡素化
}

ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processStructArrayElement(const std::string &name,
                                                  const ASTNode *literal_node) {
    return processVariableArray(name, literal_node); // 簡素化
}

ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processFunctionParameterArray(
    const std::string &name, const ASTNode *literal_node) {
    return processVariableArray(name, literal_node); // 簡素化
}

ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processFunctionReturnArray(
    const std::string &name, const ASTNode *literal_node) {
    return processVariableArray(name, literal_node); // 簡素化
}

ArrayProcessingService::ArrayOperationResult
ArrayProcessingService::processMultidimensionalArray(
    const std::string &name, const ASTNode *literal_node) {
    return processVariableArray(name, literal_node); // 簡素化
}

bool ArrayProcessingService::validateArrayContext(const std::string &name,
                                                  ArrayContext context) {
    return true; // 簡素化
}
...
void ArrayProcessingService::logArrayOperation(ArrayContext context,
