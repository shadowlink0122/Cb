...
int64_t evaluate_function_pointer_call(
    ...
    if (it == func_ptrs.end()) {
        // グローバルスコープも確認
        auto& global_func_ptrs = interpreter.get_global_scope().function_pointers;
    ...
    for (const auto& param : func_node->parameters) {
        ...
        if (param_type == TYPE_STRING) {
            ...
            interpreter.assign_function_parameter(param_name, arg_values[param_idx], param_type, is_unsigned);
        }
