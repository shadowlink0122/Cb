...
TypedValue consume_numeric_typed_value(
    ...
    if (last_captured_function_value.has_value()) {
        if (last_captured_function_value->first == node) {
            TypedValue captured = std::move(last_captured_function_value->second);
    ...
    if (resolved_type.type_name.empty()) {
        resolved_type.type_name = type_info_to_string_simple(resolved_type.type_info);
    }
...
Variable get_struct_member_from_variable(
    ...
    try {
        ...
        member_var = interpreter.get_struct_member(temp_struct_name, member_name);
        if (member_var) {
            return enforce_privacy(*member_var);
        }
    } catch (...) {
...
TypedValue evaluate_function_member_access(
    ...
    try {
        ...
        if (ret_ex.is_struct_array && ret_ex.struct_array_3d.size() > 0) {
            ...
            Variable member_var = get_struct_member_from_variable(struct_var, member_name, evaluator.get_interpreter());
...
TypedValue evaluate_function_compound_access(
    ...
    // まず配列アクセスを実行
    TypedValue array_result = evaluate_function_array_access(func_node, index_node, evaluator);
    ...
    // 構造体データからメンバーを取得
    Variable member_var = get_struct_member_from_variable(*array_result.struct_data, member_name, evaluator.get_interpreter());
...
TypedValue evaluate_recursive_member_access(
    ...
    for (size_t i = 0; i < member_path.size(); ++i) {
        ...
        // メンバーを取得
        try {
            current_var = get_struct_member_from_variable(current_var, member_name, interpreter);
