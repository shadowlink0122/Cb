...
TypedValue evaluate_binary_op_typed(
    ...
    // 未対応の演算子の場合は例外
    throw std::runtime_error("Unsupported binary operator in typed evaluation: " + node->op);
}
...
TypedValue evaluate_unary_op_typed(
    ...
    if (node->op == "ADDRESS_OF") {
        ...
        if (node->is_function_address && !node->function_address_name.empty() && !is_array_element) {
            const ASTNode* func_node = interpreter.find_function(node->function_address_name);
    ...
    if (node->op == "DEREFERENCE") {
        ...
        if (ptr_int & (1LL << 63)) {
            ...
            if (var->type == TYPE_STRUCT || var->is_struct) {
                ...
                // 浮動小数点数の場合
                InferredType deref_type(var->type, type_info_to_string_simple(var->type));
    ...
    if (node->op == "+" || node->op == "-") {
        ...
        return TypedValue(static_cast<int64_t>(operand_quad), ensure_type(inferred_type, int_like, std::string(type_info_to_string_simple(int_like))));
    } else if (node->op == "!") {
        TypedValue operand_value = evaluate_typed_func(node->left.get());
