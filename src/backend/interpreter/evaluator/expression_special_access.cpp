...
int64_t evaluate_member_array_access(
    ...
    if (member_var->is_multidimensional && indices.size() > 1) {
        if (is_function_call) {
            ...
            return interpreter.getMultidimensionalArrayElement(*member_var, indices);
        }
    }
...
int64_t evaluate_enum_access(
    ...
    std::string resolved_enum_name = interpreter.get_type_manager()->resolve_typedef(node->enum_name);
