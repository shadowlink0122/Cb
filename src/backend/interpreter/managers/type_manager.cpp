...
std::string TypeManager::resolve_typedef(const std::string &type_name) {
    ...
    if (union_it != union_definitions_.end()) {
        ...
        // 単一の基本型の場合は、その型を返す
        if (union_def.allowed_types.size() == 1 && !union_def.has_literal_values && 
...
std::string TypeManager::resolve_typedef_one_level(const std::string &type_name) {
    auto it = interpreter_->typedef_map.find(type_name);
    if (it != interpreter_->typedef_map.end()) {
...
// union typedef処理
void TypeManager::register_union_typedef(const std::string &name, const UnionDefinition &union_def) {
    extern bool debug_mode;
    if (debug_mode) {
        debug_print("REGISTER_UNION_DEBUG: Registering union typedef '%s'\n", name.c_str());
        debug_print("REGISTER_UNION_DEBUG: has_literal_values=%d, has_type_values=%d, has_custom_types=%d, has_array_types=%d\n", 
                    union_def.has_literal_values, union_def.has_type_values, union_def.has_custom_types, union_def.has_array_types);
        debug_print("REGISTER_UNION_DEBUG: allowed_values.size()=%zu, allowed_types.size()=%zu, allowed_custom_types.size()=%zu, allowed_array_types.size()=%zu\n", 
                    union_def.allowed_values.size(), union_def.allowed_types.size(), union_def.allowed_custom_types.size(), union_def.allowed_array_types.size());
    ...
    if (interpreter_->typedef_map.find(name) != interpreter_->typedef_map.end()) {
        throw std::runtime_error("Typedef name already exists: " + name);
    }
    
    // union_definitions_[name] = union_def; // この方法はデフォルトコンストラクタを呼び出すため問題
    union_definitions_.insert(std::make_pair(name, union_def)); // 直接挿入でデフォルトコンストラクタを回避
...
bool TypeManager::is_value_allowed_for_union(const std::string &type_name, const std::string &str_value) {
    ...
    if (union_def.has_literal_values) {
        for (const auto &allowed : union_def.allowed_values) {
            if (allowed.value_type == TYPE_STRING && allowed.string_value == str_value) {
                return true;
            }
        }
...
bool TypeManager::is_value_allowed_for_union(const std::string &type_name, int64_t int_value) {
    auto it = union_definitions_.find(type_name);
    if (it == union_definitions_.end()) {
        return false;
    }
    ...
    extern bool debug_mode;
    if (debug_mode) {
        debug_print("UNION_TYPE_DEBUG: Checking int value %lld for union type %s\n", int_value, type_name.c_str());
        debug_print("UNION_TYPE_DEBUG: has_literal_values=%d, has_type_values=%d\n", 
                    union_def.has_literal_values, union_def.has_type_values);
        debug_print("UNION_TYPE_DEBUG: allowed_values.size()=%zu, allowed_types.size()=%zu\n", 
    ...
    if (union_def.has_literal_values) {
        ...
        for (const auto &allowed : union_def.allowed_values) {
            if (debug_mode) {
                debug_print("UNION_TYPE_DEBUG: Comparing with allowed value: type=%d, int_value=%lld, bool_value=%d\n", 
                            static_cast<int>(allowed.value_type), allowed.int_value, allowed.bool_value);
            }
            if ((allowed.value_type == TYPE_INT || allowed.value_type == TYPE_LONG || 
                 allowed.value_type == TYPE_SHORT || allowed.value_type == TYPE_TINY || 
                 allowed.value_type == TYPE_CHAR) && 
                allowed.int_value == int_value) {
                return true;
            }
            // boolean値もチェック
            if (allowed.value_type == TYPE_BOOL && allowed.bool_value == (int_value == 1)) {
                return true;
            }
        }
        ...
        if (debug_mode) {
            debug_print("UNION_TYPE_DEBUG: Literal value %lld not found in allowed values\n", int_value);
        }
        
        // 値が数値で、かつint系型が許可されている場合のみ型チェックに進む
        bool has_int_types = union_def.is_type_allowed(TYPE_INT) || union_def.is_type_allowed(TYPE_LONG) ||
                            union_def.is_type_allowed(TYPE_SHORT) || union_def.is_type_allowed(TYPE_TINY) ||
                            union_def.is_type_allowed(TYPE_CHAR);
        
        if (has_int_types) {
            if (debug_mode) {
                debug_print("UNION_TYPE_DEBUG: Mixed union has int types, checking if value fits\n");
            }
        } else {
            // int系の型が許可されておらず、リテラル値も一致しない場合はfalse
            if (debug_mode) {
                debug_print("UNION_TYPE_DEBUG: No int types allowed and literal value not found, returning false\n");
            }
            return false;
        }
    }
    ...
    if (union_def.has_literal_values) {
        ...
        if (basic_type_result) {
            if (debug_mode) {
                debug_print("UNION_TYPE_DEBUG: Mixed union with literals and explicit int type - allowing int value %lld\n", int_value);
            }
            return true;
        } else {
            // 基本型が明示的に許可されていない場合は、リテラル値のみ
            if (debug_mode) {
                debug_print("UNION_TYPE_DEBUG: Mixed union with literals but no explicit int type - rejecting int value %lld\n", int_value);
            }
            return false;
        }
    }
    ...
    if (!basic_type_result && union_def.has_custom_types) {
        if (debug_mode) {
            debug_print("UNION_CUSTOM_TYPE_DEBUG: Checking custom types for int value %lld\n", int_value);
            debug_print("UNION_CUSTOM_TYPE_DEBUG: Number of custom types: %zu\n", union_def.allowed_custom_types.size());
        }
        for (const auto& custom_type : union_def.allowed_custom_types) {
            if (debug_mode) {
                debug_print("UNION_CUSTOM_TYPE_DEBUG: Examining custom type '%s'\n", custom_type.c_str());
            }
            // カスタム型がint系の基本型へのtypedefかどうかをチェック
            std::string resolved_type = resolve_typedef(custom_type);
            if (debug_mode) {
                debug_print("UNION_CUSTOM_TYPE_DEBUG: Custom type '%s' resolves to '%s'\n", 
                            custom_type.c_str(), resolved_type.c_str());
            }
            if (resolved_type == "int" || resolved_type == "long" || 
                resolved_type == "short" || resolved_type == "tiny" || resolved_type == "char") {
                if (debug_mode) {
                    debug_print("UNION_CUSTOM_TYPE_DEBUG: Custom type '%s' is int-compatible, returning true\n", 
                                custom_type.c_str());
                }
                return true;
            }
        ...
        if (debug_mode) {
            debug_print("UNION_CUSTOM_TYPE_DEBUG: No int-compatible custom types found\n");
        }
    }
...
bool TypeManager::is_value_allowed_for_union(const std::string &type_name, bool bool_value) {
    auto it = union_definitions_.find(type_name);
    if (it == union_definitions_.end()) {
        return false;
    }
    ...
    if (union_def.has_literal_values) {
        for (const auto &allowed : union_def.allowed_values) {
            if (allowed.value_type == TYPE_BOOL && allowed.bool_value == bool_value) {
                return true;
            }
        }
        // リテラル値ユニオンで値が見つからなかった場合はfalseを返す
...
bool TypeManager::is_custom_type_allowed_for_union(const std::string &union_type_name, const std::string &custom_type_name) {
    ...
    extern bool debug_mode;
    if (debug_mode) {
        debug_print("UNION_CUSTOM_TYPE_DEBUG: Checking custom type '%s' for union type %s\n", custom_type_name.c_str(), union_type_name.c_str());
        debug_print("UNION_CUSTOM_TYPE_DEBUG: has_custom_types=%d, allowed_custom_types.size()=%zu\n", 
                    union_def.has_custom_types, union_def.allowed_custom_types.size());
    }
    
    bool result = union_def.is_custom_type_allowed(custom_type_name);
    if (debug_mode) {
        debug_print("UNION_CUSTOM_TYPE_DEBUG: is_custom_type_allowed('%s') returned %d\n", 
...
bool TypeManager::is_array_type_allowed_for_union(const std::string &union_type_name, const std::string &array_type) {
    auto it = union_definitions_.find(union_type_name);
    if (it == union_definitions_.end()) {
        return false;
    }
    ...
    extern bool debug_mode;
    if (debug_mode) {
        debug_print("UNION_ARRAY_TYPE_DEBUG: Checking array type '%s' for union type %s\n", array_type.c_str(), union_type_name.c_str());
        debug_print("UNION_ARRAY_TYPE_DEBUG: has_array_types=%d, allowed_array_types.size()=%zu\n", 
                    union_def.has_array_types, union_def.allowed_array_types.size());
    }
