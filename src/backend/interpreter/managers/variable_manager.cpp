...
void VariableManager::process_variable_declaration(const ASTNode *node) {
        ...
        // typedef解決処理（ArrayTypeInfoが設定されていない場合）
            ...
            if (node->right || node->init_expr) {
                ...
                if (var.type == TYPE_STRING &&
                    ...
                    throw std::runtime_error(
                        ...
                        node->type_name + "'");
                           ...
                           init_node->node_type == ASTNodeType::AST_NUMBER &&
                           init_node->int_value != 0 &&
                           init_node->int_value != 1) {
                    throw std::runtime_error(
                        "Type mismatch: Cannot assign integer value " +
                        std::to_string(init_node->int_value) +
                ...
                if (var.type == TYPE_STRING &&
                    ...
                    // 左オペランドを取得
                    if (init_node->left->node_type ==
                        ASTNodeType::AST_VARIABLE) {
                        Variable *left_var =
                            find_variable(init_node->left->name);
                        if (left_var &&
                            (left_var->type == TYPE_STRING ||
                    ...
                    try {
                        int64_t value = interpreter_->expression_evaluator_
                                            ->evaluate_expression(init_node);
                        clamp_unsigned_value(var, value, "initialized with expression", node);
                        var.value = value;
                        var.is_assigned = true;
                        ...
                        throw std::runtime_error(
                            "Failed to initialize typedef variable '" +
                            node->name + "': " + e.what());
                    }
                }
            }
                ...
                if (bracket_pos != std::string::npos) {
                    ...
                    if (close_bracket_pos != std::string::npos) {
                        ...
                        if (!size_str.empty()) {
                            ...
                            struct_array_dimensions.push_back(
                                struct_array_size);
                        }
                    }
                    ...
                    for (const auto &member : struct_def->members) {
                        ...
                        if (member.array_info.is_array()) {
                            ...
                            if (member_var.array_dimensions.size() > 1) {
                                ...
                                for (const auto &dim_size :
                                    ...
                                    ArrayDimension dimension(
                                        dim_size,
                                        false); // 構造体メンバーは静的サイズ
                                    member_var.array_type_info.dimensions
                                        .push_back(dimension);
                                }
                            ...
                            for (int i = 0; i < total_size; i++) {
                                ...
                                std::string element_type_alias =
                                    member.type_alias;

                                // type_aliasから配列のサイズ情報を削除（例:
                                // "Point[3]" -> "Point"）
                                size_t bracket_pos =
                                    element_type_alias.find('[');
                                if (bracket_pos != std::string::npos) {
                                    element_type_alias =
                                        element_type_alias.substr(0,
                                                                  bracket_pos);
                                }
                                ...
                                if (element_type_info == TYPE_STRUCT &&
                                    ...
                                    if (element_struct_def) {
                                        for (const auto &element_member :
                                            ...
                                            element_member_var.is_assigned =
                                                false;
                                ...
                                if (interpreter_->debug_mode) {
                                    debug_print(
                                        "Created struct member array element: "
                                        "%s (key: %s), is_struct=%s, "
                                        "members=%zu\n",
                                        element_name.c_str(),
                                        element_key.c_str(),
                        ...
                        if (member.type == TYPE_STRUCT &&
                            ...
                            if (interpreter_->debug_mode) {
                                debug_print(
                                    "Recursively creating nested struct "
                                    "members for: %s (type: %s)\n",
                                    member_path.c_str(),
                    ...
                    if (ret.is_struct) {
                        ...
                        if (interpreter_->debug_mode &&
                            ...
                            if (scores_it !=
                                ...
                                if (scores_it->second.array_values.size() >=
                                    3) {
                                    debug_print(
                                        "FUNC_RETURN_RECEIVED: "
                        ...
                        if (ret.type == TYPE_STRING) {
                            ...
                            if (!ret.double_array_3d.empty() &&
                                ...
                                std::string actual_type =
                                    interpreter_->type_manager_
                                        ->resolve_typedef(ret.array_type_name);
                                bool is_multidim =
                                    (actual_type.find("[][]") !=
                                         std::string::npos ||
                ...
                if (var.type == TYPE_STRING &&
                    ...
                    try {
                        ...
                        if (typed_result.is_string()) {
                            ...
                            } else if (typed_result.numeric_type ==
                                       TYPE_DOUBLE) {
                                double d = static_cast<double>(quad_val);
        ...
        if (node->type_info == TYPE_POINTER) {
            ...
            if (node->init_expr || node->right) {
                ...
                if (interpreter_->debug_mode) {
                    std::cerr
                        << "[VAR_MANAGER] Checking pointer init: node_type="
