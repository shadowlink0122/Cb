...
class TypeManager {
  private:
    ...
    std::map<std::string, UnionDefinition>
        union_definitions_; // union typedef definitions
  ...
  public:
    ...
    // union typedef処理
    void register_union_typedef(const std::string &name,
    ...
    bool is_value_allowed_for_union(const std::string &type_name,
                                    const std::string &str_value);
    bool is_value_allowed_for_union(const std::string &type_name,
                                    int64_t int_value);
    bool is_value_allowed_for_union(const std::string &type_name,
                                    bool bool_value);
    bool is_custom_type_allowed_for_union(const std::string &union_type_name,
                                          const std::string &custom_type_name);
    bool is_array_type_allowed_for_union(const std::string &union_type_name,
