...
EnumManager::get_enum_definition(const std::string &enum_name) const {
...
bool EnumManager::get_enum_value(const std::string &enum_name,
    ...
    if (!member) {
        DEBUG_ERROR(GENERAL, "Enum member %s::%s not found", enum_name.c_str(),
                    member_name.c_str());
        return false;
    }

    value = member->value;
    DEBUG_DEBUG(GENERAL, "Found enum value %s::%s = %lld", enum_name.c_str(),
                member_name.c_str(), value);
    return true;
}

bool EnumManager::validate_enum_definition(const EnumDefinition &definition,
    ...
    for (size_t i = 0; i < definition.members.size(); ++i) {
        for (size_t j = i + 1; j < definition.members.size(); ++j) {
            if (definition.members[i].name == definition.members[j].name) {
                std::ostringstream oss;
                oss << "Duplicate enum member name '"
                    << definition.members[i].name << "'";
                error_message = oss.str();
                return false;
            }
        }
...
bool EnumManager::has_duplicate_values_in_definition(
