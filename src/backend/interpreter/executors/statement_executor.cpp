...
void StatementExecutor::execute_assignment(const ASTNode *node) {
    ...
    if (node->right &&
        ...
        if (node->left->node_type == ASTNodeType::AST_VARIABLE) {
            ...
            interpreter_.assign_struct_literal(node->left->name,
                                               node->right.get());
            return;
            ...
            std::string element_name =
                interpreter_.extract_array_element_name(node->left.get());
            if (debug_mode) {
    ...
    if (node->left && node->left->node_type == ASTNodeType::AST_ARRAY_REF) {
        ...
        if (node->right &&
            ...
            try {
                ...
                if (ret.is_struct) {
                    // 構造体戻り値を配列要素に代入
                    std::string element_name =
                        interpreter_.extract_array_element_name(
                            node->left.get());
                    debug_msg(
        ...
        try {
            TypedValue typed_rvalue =
                interpreter_.evaluate_typed_expression(node->right.get());
        ...
        if (node->left->left &&
            ...
            int64_t index =
                interpreter_.evaluate(node->left->array_index.get());
        ...
        if (node->left->left &&
            ...
            if (deepest_left->left && deepest_left->left->node_type ==
                ...
                for (auto it = refs.rbegin(); it != refs.rend(); ++it) {
                    ...
                    int64_t index =
                        interpreter_.evaluate((*it)->array_index.get());
                ...
                // 通常の多次元配列要素への代入
                std::string var_name =
                    interpreter_.extract_array_name(node->left.get());
                std::vector<int64_t> indices =
                    interpreter_.extract_array_indices(node->left.get());
                ...
                if (is_floating &&
                    (base_type == TYPE_FLOAT || base_type == TYPE_DOUBLE ||
                     base_type == TYPE_QUAD)) {
                    interpreter_.setMultidimensionalArrayElement(*var, indices,
                                                                 float_rvalue);
                } else {
                    interpreter_.setMultidimensionalArrayElement(*var, indices,
                                                                 rvalue);
            ...
            if (var->type == TYPE_STRING) {
                ...
                if (is_floating &&
                    (base_type == TYPE_FLOAT || base_type == TYPE_DOUBLE ||
                     base_type == TYPE_QUAD)) {
                    interpreter_.assign_array_element_float(var_name, index,
                                                            float_rvalue);
                } else {
        ...
        if (node->left && node->left->node_type == ASTNodeType::AST_VARIABLE) {
            ...
            if (var && var->type == TYPE_UNION) {
                interpreter_.assign_union_variable(node->left->name,
                                                   node->right.get());
                return;
            }
        }
        ...
        if (!node->name.empty()) {
            ...
            if (var && var->type == TYPE_UNION) {
                interpreter_.assign_union_variable(node->name,
                                                   node->right.get());
                return;
            }
        }
        ...
        if (node->right &&
            ...
            TypeInfo type_hint = (typed_value.numeric_type == TYPE_POINTER)
                                     ? TYPE_POINTER
                                     : TYPE_UNKNOWN;
            interpreter_.assign_variable(target_name, typed_value, type_hint,
                                         false);
        }
    }
}
...
void StatementExecutor::execute_variable_declaration(const ASTNode *node) {
    ...
    // union型の特別処理
    if (!node->type_name.empty() &&
        interpreter_.get_type_manager()->is_union_type(node->type_name)) {
    ...
    if (init_node) {
        ...
        if (node->type_info == TYPE_POINTER) {
            ...
            interpreter_.current_scope().variables[node->name].value =
                typed_value.value;
            interpreter_.current_scope().variables[node->name].type =
                TYPE_POINTER;
            ...
            try {
                ...
                if (ret.is_array) {
                    // 配列戻り値の場合
                    Variable &target_var =
                        interpreter_.current_scope().variables[node->name];
                    ...
                    if (ret.is_struct) {
                        ...
                        if (ret.type == TYPE_FLOAT) {
                            ...
                            interpreter_.assign_variable(node->name, typed_val,
                                                         ret.type, false);
                        } else if (ret.type == TYPE_DOUBLE) {
            ...
            if (init_node->node_type == ASTNodeType::AST_FUNC_CALL) {
                try {
                    TypedValue typed_value =
                        interpreter_.evaluate_typed(init_node);
