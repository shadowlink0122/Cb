...
class OutputManager {
  public:
    ...
    std::string format_string(const ASTNode *format_str,
                              const ASTNode *arg_list);
  ...
  private:
    ...
    ExpressionService
        *expression_service_; // DRY効率化:
                              // 統一式評価サービス（Interpreterから取得）
    ...
    std::string render_formatted_string(
    ...
    void collect_formatted_arguments(const ASTNode *arg_list,
                                     size_t start_index, bool allow_print_nodes,
