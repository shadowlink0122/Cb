...
void Interpreter::execute_statement(const ASTNode *node) {
    ...
    switch (node->node_type) {
    case ASTNodeType::AST_STMT_LIST:
        debug_msg(DebugMsgId::INTERPRETER_STMT_LIST_EXEC,
                  node->statements.size());
        if (debug_mode) {
            std::cerr << "[STMT_LIST_DEBUG] Processing "
                      << node->statements.size() << " statements" << std::endl;
        }
        ...
        if (node->left) {
            ...
            if (node->left->node_type == ASTNodeType::AST_ARRAY_LITERAL) {
                const std::vector<std::unique_ptr<ASTNode>> &elements =
                    node->left->arguments;
                debug_msg(DebugMsgId::INTERPRETER_RETURN_ARRAY,
                          elements.size());
                // 配列リテラルを処理
                ...
                if (!elements.empty() &&
                    ...
                    if (is_string_array) {
                        ...
                        for (const auto &row_element : elements) {
                            if (row_element->node_type !=
                                ...
                                throw std::runtime_error(
                                    "Expected nested array literal");
                            }

                            std::vector<std::string> row;
                            for (const auto &cell_element :
                                 row_element->arguments) {
                                if (cell_element->node_type !=
                                    ASTNodeType::AST_STRING_LITERAL) {
                                    throw std::runtime_error(
                                        "Expected string literal in multidim "
                                        "array");
                                }
                        ...
                        throw ReturnException(int_array_3d, "int[][]",
                                              TYPE_INT);
                    }
                }
                ...
                if (node->left->name == "self") {
                    ...
                    if (self_var && self_var->is_struct) {
                        ...
                        for (const auto &member : self_var->struct_members) {
                            debug_print(
                                "RETURN_SELF: Before sync member %s = %lld\n",
                        ...
                        for (const auto &member : self_var->struct_members) {
                            debug_print(
                                "RETURN_SELF: After sync member %s = %lld\n",
                                member.first.c_str(), member.second.value);
                        }
                        ...
                        debug_print("RETURN_SELF: struct_type=%s, type=%d, "
                        ...
                        debug_print("RETURN_SELF: self not found or not struct "
                                    "(found=%d, is_struct=%d)\n",
                                    self_var != nullptr,
                                    self_var ? self_var->is_struct : 0);
                    }
                } else {
                    // 他の識別子の場合、変数として扱う
                    ...
                    if (var) {
                        ...
                        if (!current_function_name.empty()) {
                            const ASTNode *func =
                                global_scope.functions[current_function_name];
                            if (func && func->return_type_name.find('&') !=
                                            std::string::npos) {
                ...
                debug_msg(DebugMsgId::INTERPRETER_RETURN_VAR,
                ...
                if (var && var->is_struct) {
                    ...
                    if (debug_mode) {
                        ...
                        if (scores_it != var->struct_members.end() &&
                            scores_it->second.is_array) {
                            if (scores_it->second.array_values.size() >= 3) {
                                debug_print("RETURN_BEFORE_SYNC: "
                                            "%s.scores.array_values = [%lld, "
                                            "%lld, %lld]\n",
                                            node->left->name.c_str(),
                    ...
                    if (debug_mode) {
                        ...
                        if (scores_it != var->struct_members.end() &&
                            ...
                            if (scores_it->second.array_values.size() >= 3) {
                                debug_print(
                                    "RETURN_AFTER_SYNC: %s.scores.array_values "
                                    "= [%lld, %lld, %lld]\n",
                                    node->left->name.c_str(),
                                    scores_it->second.array_values[0],
                                    scores_it->second.array_values[1],
                    ...
                    if (var->is_array) {
                        ...
                        for (int i = 0; i < var->array_size; ++i) {
                            std::string element_name = node->left->name + "[" +
                                                       std::to_string(i) + "]";
                            ...
                            if (element_var && element_var->is_struct) {
                                ...
                                debug_msg(
                                    DebugMsgId::INTERPRETER_ARRAY_ELEMENT, i,
                        ...
                        std::string struct_type_name = var->type_name.empty()
                                                           ? node->left->name
                                                           : var->type_name;
                        throw ReturnException(struct_array_3d,
                                              struct_type_name);
                    }
                    ...
                    if (var->is_multidimensional) {
                        debug_msg(DebugMsgId::INTERPRETER_MULTIDIM_ARRAY_SIZE,
                                  var->multidim_array_values.size());
                    } else {
                        debug_msg(DebugMsgId::INTERPRETER_REGULAR_ARRAY_SIZE,
                    ...
                    if (var->is_multidimensional) {
                        ...
                        // 文字列配列の場合
                        if (var->type == static_cast<TypeInfo>(TYPE_ARRAY_BASE +
                                                               TYPE_STRING) ||
                            ...
                            if (var->array_dimensions.size() == 2) {
                                ...
                                for (size_t i = 0;
                                     i < var->multidim_array_strings.size();
                                     ++i) {
                                    str_array_1d.push_back(
                            ...
                            throw ReturnException(str_array_3d,
                                                  node->left->name, var->type);
                        }
                        ...
                        TypeInfo base_type =
                            (type_info >= TYPE_ARRAY_BASE)
                                ? static_cast<TypeInfo>(type_info -
                                                        TYPE_ARRAY_BASE)
                                : type_info;
                        ...
                        if (base_type == TYPE_FLOAT ||
                            ...
                            if (var->array_dimensions.size() == 2) {
                                ...
                                if (base_type == TYPE_FLOAT) {
                                    for (size_t i = 0;
                                         i < var->multidim_array_float_values
                                                 .size();
                                         ++i) {
                                    ...
                                    for (size_t i = 0;
                                         i < var->multidim_array_double_values
                                                 .size();
                                         ++i) {
                                        double_array_1d.push_back(
                                            var->multidim_array_double_values
                            ...
                            throw ReturnException(double_array_3d,
                                                  node->left->name, base_type);
                        }
                        ...
                        if (var->array_dimensions.size() == 2) {
                            ...
                            for (int i = 0; i < rows; i++) {
                                ...
                                for (int j = 0; j < cols; j++) {
                                    ...
                                    if (flat_index <
                                        var->multidim_array_values.size()) {
                                        row.push_back(var->multidim_array_values
                                                          [flat_index]);
                                        debug_msg(
                                            DebugMsgId::
                            ...
                            for (size_t i = 0;
                                ...
                                debug_msg(
                                    DebugMsgId::INTERPRETER_MULTIDIM_ELEMENT,
                                    (int)i,
                    ...
                    if (base_type == TYPE_FLOAT || base_type == TYPE_DOUBLE ||
                        ...
                        if (var->is_multidimensional &&
                            ...
                            if (base_type == TYPE_FLOAT) {
                                for (size_t i = 0;
                                     i < var->array_float_values.size(); ++i) {
                                    double_array_1d.push_back(
                                        static_cast<double>(
                                            var->array_float_values[i]));
                                }
                            } else if (base_type == TYPE_DOUBLE) {
                                for (size_t i = 0;
                                     i < var->array_double_values.size(); ++i) {
                                    double_array_1d.push_back(
                                        var->array_double_values[i]);
                                }
                            } else { // TYPE_QUAD
                                for (size_t i = 0;
                                     i < var->array_quad_values.size(); ++i) {
                                    double_array_1d.push_back(
                                        static_cast<double>(
                                            var->array_quad_values[i]));
                                }
                            }
                            double_array_2d.push_back(double_array_1d);
                            double_array_3d.push_back(double_array_2d);
                        ...
                        throw ReturnException(double_array_3d, type_name,
                                              base_type);
                    } else if (type_info == static_cast<TypeInfo>(
                                                TYPE_ARRAY_BASE + TYPE_INT) ||
                               type_info == static_cast<TypeInfo>(
                                                TYPE_ARRAY_BASE + TYPE_LONG) ||
                               type_info == static_cast<TypeInfo>(
                                                TYPE_ARRAY_BASE + TYPE_SHORT) ||
                               type_info == static_cast<TypeInfo>(
                                                TYPE_ARRAY_BASE + TYPE_TINY) ||
                               type_info == static_cast<TypeInfo>(
                                                TYPE_ARRAY_BASE + TYPE_BOOL)) {
                        // 整数配列を3D配列として格納
                        std::vector<std::vector<std::vector<int64_t>>>
                            int_array_3d;
                        std::vector<std::vector<int64_t>> int_array_2d;
                        ...
                        for (size_t i = 0; i < var->array_values.size(); ++i) {
                            ...
                            debug_msg(DebugMsgId::INTERPRETER_ARRAY_ELEMENT,
                                      (int)i, (long long)var->array_values[i]);
                        }
                        int_array_2d.push_back(int_array_1d);
                        int_array_3d.push_back(int_array_2d);
                        ...
                        for (int i = 0; i < var->array_size; ++i) {
                            ...
                            if (element_var && element_var->is_struct) {
                                ...
                                for (const auto &member_pair :
                                    ...
                                    struct_element
                                        .struct_members[member_pair.first] =
                                        member_pair.second;
                                }
                ...
                if (node->left->node_type == ASTNodeType::AST_VARIABLE) {
                    ...
                    if (var && var->is_struct) {
                        ...
                        if (typed_result.numeric_type == TYPE_FLOAT) {
                            throw ReturnException(typed_result.double_value,
                                                  TYPE_FLOAT);
                        } else if (typed_result.numeric_type == TYPE_DOUBLE) {
                            throw ReturnException(typed_result.double_value,
                                                  TYPE_DOUBLE);
                        } else if (typed_result.numeric_type == TYPE_QUAD) {
                            throw ReturnException(typed_result.quad_value,
                                                  TYPE_QUAD);
                        } else {
                        ...
                        try {
                            ...
                            if (member_var && member_var->type == TYPE_STRING) {
                                ...
                                if (member_var->type == TYPE_FLOAT) {
                                    ...
                                    throw ReturnException(
                                        member_var->double_value, TYPE_DOUBLE);
                                } else if (member_var->type == TYPE_QUAD) {
                                    throw ReturnException(
                                        member_var->quad_value, TYPE_QUAD);
                                } else {
                    ...
                    if (node->left->node_type == ASTNodeType::AST_VARIABLE &&
                        node->left->name == "self") {
                        debug_print("RETURN_SELF_DEBUG: Processing return self "
                    ...
                    std::cerr << "DEBUG: typed_result.is_struct_result = "
                              << typed_result.is_struct_result << std::endl;

                    if (typed_result.is_function_pointer) {
                        // 関数ポインタを返す
                        if (debug_mode) {
                            std::cerr << "[RETURN] Function pointer: "
                                      << typed_result.function_pointer_name
                                      << " -> " << typed_result.value
                                      << std::endl;
                        }
                        throw ReturnException(
                            typed_result.value,
                            typed_result.function_pointer_name,
                            typed_result.function_pointer_node,
                            typed_result.numeric_type);
                    } else if (typed_result.is_struct_result) {
                        std::cerr << "DEBUG: Struct result detected, "
                        ...
                        try {
                            expression_evaluator_->evaluate_expression(
                                node->left.get());
                            throw std::runtime_error(
                                "Struct evaluation did not throw "
                                "ReturnException");
                        } catch (const ReturnException &ret_ex) {
                            std::cerr << "DEBUG: Successfully caught "
                                         "ReturnException for struct"
                                      << std::endl;
                ...
                if (node->left->node_type == ASTNodeType::AST_VARIABLE) {
                    ...
                    if (result.is_function_pointer) {
                        // 関数ポインタを返す
                        if (debug_mode) {
                            std::cerr << "[RETURN else] Function pointer: "
                                      << result.function_pointer_name << " -> "
                                      << result.value << std::endl;
                        }
                        throw ReturnException(
                            result.value, result.function_pointer_name,
                        ...
                        if (result.numeric_type == TYPE_FLOAT) {
                            throw ReturnException(result.double_value,
                                                  TYPE_FLOAT);
...
void Interpreter::handle_impl_declaration(const ASTNode *node) {
    ...
    for (const auto &static_var_node : node->impl_static_variables) {
        ...
        create_impl_static_variable(static_var_node->name,
                                    static_var_node.get());
        exit_impl_context();
    }
...
void Interpreter::create_struct_variable(const std::string &var_name,
    ...
    const StructDefinition *struct_def = find_struct_definition(
    ...
    for (const auto &member : struct_def->members) {
        ...
        if (member.array_info.is_array()) {
            ...
            if (member.array_info.dimensions.size() > 1) {
                // 多次元配列メンバ
                if (debug_mode) {
                    debug_print("Creating multidimensional array member: %s "
                                "with %zu dimensions\n",
                                member.name.c_str(),
                                member.array_info.dimensions.size());
                }
                ...
                for (const auto &dim : member.array_info.dimensions) {
                    ...
                    if (dim_size <= 0) {
                        throw std::runtime_error(
                            "Invalid dimension size for struct member " +
                            member.name);
                    }
                ...
                // フラットな配列として初期化
                multidim_array_member.multidim_array_values.resize(total_size,
                                                                   0);
                if (member.type == TYPE_STRING) {
                    multidim_array_member.multidim_array_strings.resize(
                        total_size, "");
                }
                ...
                if (array_size == -1 &&
                    ...
                    if (const_var) {
                        ...
                        if (const_var->is_const && const_var->is_assigned) {
                            ...
                            if (debug_mode) {
                                debug_print(
                                    "Constant %s found but not const or "
                                    "not assigned\n",
                                    member.array_info.dimensions[0]
                                        .size_expr.c_str());
                            }
                ...
                if (array_size <= 0) {
                    throw std::runtime_error(
                        "Invalid array size (" + std::to_string(array_size) +
                ...
                if (debug_mode) {
                    debug_print(
                        "Added to struct_members: %s, final array_size=%d\n",
                        member.name.c_str(),
                ...
                if (debug_mode) {
                    debug_print("Added array member to struct_members, final "
                                "array_values.size()=%zu\n",
                                struct_var.struct_members[member.name]
                                    .array_values.size());
                }
                ...
                for (int i = 0; i < array_size; i++) {
                    ...
                    if (member.type == TYPE_STRUCT &&
                        ...
                        if (debug_mode) {
                            debug_print("Creating struct array element: %s[%d] "
                                        "of type %s\n",
                                        member.name.c_str(), i,
                        ...
                        if (element_struct_def) {
                            ...
                            if (debug_mode) {
                                debug_print(
                                    "Initialized struct array element with %zu "
                                    "members\n",
            ...
            if (member.type == TYPE_STRUCT && !member.type_alias.empty()) {
                ...
                if (nested_struct_def) {
                    if (debug_mode) {
                        debug_print("Initializing nested struct member: %s "
                                    "with %zu members\n",
                                    member.name.c_str(),
                                    nested_struct_def->members.size());
                    }

                    // ネストした構造体のメンバーを再帰的に初期化
                    for (const auto &nested_member :
                         nested_struct_def->members) {
                        Variable nested_member_var;
                        nested_member_var.type = nested_member.type;
                        nested_member_var.is_unsigned =
                            nested_member.is_unsigned;
                        nested_member_var.is_private_member =
                            nested_member.is_private;
                        ...
                        member_var.struct_members[nested_member.name] =
                            nested_member_var;

                        // 個別変数としても登録
                        std::string nested_member_path = var_name + "." +
                                                         member.name + "." +
                                                         nested_member.name;
                        current_scope().variables[nested_member_path] =
                            nested_member_var;
                    ...
                    if (debug_mode) {
                        debug_print("Warning: Nested struct definition not "
                                    "found for type: %s\n",
                                    resolved_type.c_str());
                    }
                }
    ...
    if (registered_var && registered_var->is_struct) {
        if (debug_mode) {
            debug_print(
                "Post-registration: adding array elements to struct_members\n");
        }
        ...
        for (const auto &member : struct_def->members) {
            if (member.array_info.is_array() &&
                ...
                for (int i = 0; i < array_size; i++) {
                    ...
                    if (element_var) {
                        registered_var->struct_members[element_key] =
                            *element_var;

                        if (debug_mode) {
                            debug_print(
                                "Added array element to struct_members: %s\n",
                                element_key.c_str());
                        }
                    } else {
                        if (debug_mode) {
...
void Interpreter::create_struct_member_variables_recursively(
    ...
    for (const auto &member_def : struct_def->members) {
        ...
        current_scope().variables[full_member_path] =
            parent_var.struct_members[member_def.name];
    }
        ...
        if (direct_var) {
            ...
            if (member.type >= TYPE_ARRAY_BASE ||
                ...
                bool direct_is_union =
                    type_manager_->is_union_type(*direct_var);
                bool member_is_union =
                    (!member_union_alias.empty() &&
                     type_manager_->is_union_type(member_union_alias));
    ...
    if (existing != impl_definitions_.end()) {
        *existing = stored_def;
        ...
        debug_print("IMPL_DEF_STORAGE: Added new impl '%s' for '%s' (total: "
                    ...
                    (void *)&impl_definitions_, (void *)this);
    }
