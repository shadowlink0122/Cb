...
struct PointerMetadata {
    ...
    PointerMetadata()
        : target_type(PointerTargetType::NULLPTR_VALUE), address(0),
          ...
          array_start_addr(0), array_end_addr(0), var_ptr(nullptr),
    ...
    // ヘルパーメソッド
    bool is_null() const {
        return target_type == PointerTargetType::NULLPTR_VALUE;
    }
    bool is_variable() const {
        return target_type == PointerTargetType::VARIABLE;
    }
    bool is_array_element() const {
        return target_type == PointerTargetType::ARRAY_ELEMENT;
    }
    bool is_struct_member() const {
        return target_type == PointerTargetType::STRUCT_MEMBER;
    }
...
struct PointerValue {
    ...
    // 従来の方式用コンストラクタ
    explicit PointerValue(int64_t ptr)
        : has_metadata(false), raw_pointer(ptr) {}

    // 新しい方式用コンストラクタ
    explicit PointerValue(PointerMetadata *meta)
        : has_metadata(true), metadata(meta) {}
