...
ParsedTypeString parse_type_string(const std::string &type_name) {
    ...
    while (end_pos > 0 && std::isspace(static_cast<unsigned char>(
                              without_arrays[end_pos - 1]))) {
        --end_pos;
    }
    ...
    while (pointer_pos > 0) {
        ...
        if (ch == '*') {
            ++pointer_depth;
            --pointer_pos;
            while (pointer_pos > 0 && std::isspace(static_cast<unsigned char>(
                                          without_arrays[pointer_pos - 1]))) {
                --pointer_pos;
            }
        } else {
            break;
...
InferredType TypeInferenceEngine::infer_type(const ASTNode *node) {
    ...
    case ASTNodeType::AST_ARRAY_LITERAL: {
        ...
        if (!node->arguments.empty()) {
            ...
            int element_dims =
                element_type.is_array ? element_type.array_dimensions : 0;
            ...
            std::string array_name =
                build_array_type_name(base_name, total_dims);
            return InferredType(element_type.type_info, array_name, true,
                                total_dims);
        }
    ...
    case ASTNodeType::AST_VARIABLE: {
        // 変数の型を取得
        auto *var =
            interpreter_.get_variable_manager()->find_variable(node->name);
        if (var) {
            TypeInfo stored_type = var->type;
            int dims = 0;
            if (var->array_type_info.is_array()) {
                dims = static_cast<int>(
                    var->array_type_info.get_dimension_count());
            ...
            if (stored_type >= TYPE_ARRAY_BASE) {
                base_type =
                    static_cast<TypeInfo>(stored_type - TYPE_ARRAY_BASE);
            }

            if (var->array_type_info.is_array() &&
                var->array_type_info.base_type != TYPE_UNKNOWN) {
            ...
            if (type_manager && !base_name.empty() &&
                stored_type != TYPE_UNION) {
            ...
            if (base_name.empty()) {
                if (base_type == TYPE_STRUCT &&
                    ...
                    base_name = var->struct_type_name;
                } else if (stored_type == TYPE_UNION &&
                           !var->type_name.empty()) {
                    base_name = var->type_name;
                } else {
            ...
            if (dims == 0 &&
                (var->is_array || stored_type >= TYPE_ARRAY_BASE)) {
                dims = 1;
            }

            bool is_array = dims > 0;
            std::string type_name =
                is_array ? build_array_type_name(base_name, dims) : base_name;
    ...
    case ASTNodeType::AST_TERNARY_OP:
        return infer_ternary_type(node->left.get(), node->right.get(),
                                  node->third.get());
...
InferredType
TypeInferenceEngine::infer_ternary_type(const ASTNode *condition,
...
InferredType TypeInferenceEngine::infer_function_return_type(
    const std::string &func_name, const std::vector<InferredType> &arg_types) {
    // 関数定義を検索して戻り値型を取得
    ...
    if (func_def) {
        ...
        if (!func_def->return_types.empty()) {
            ...
            bool is_array_hint =
                func_def->is_array_return || stored_type >= TYPE_ARRAY_BASE;
            int array_dims_hint = 0;

            if (!func_def->return_type_name.empty()) {
                ParsedTypeString parsed =
                    parse_type_string(func_def->return_type_name);
            ...
            if (type_name_hint.empty()) {
                if (stored_type >= TYPE_ARRAY_BASE) {
                    TypeInfo base_info =
                        static_cast<TypeInfo>(stored_type - TYPE_ARRAY_BASE);
                    type_name_hint =
                        build_array_type_name(type_info_to_string(base_info),
                                              std::max(1, array_dims_hint));
                } else {
            ...
            if (is_array_hint) {
                ...
                inferred.array_dimensions =
                    (array_dims_hint > 0)
                        ? array_dims_hint
                        : std::max(1, inferred.array_dimensions);
                std::string base_name = remove_array_suffix(
                    inferred.type_name, inferred.array_dimensions);
                ...
                inferred.type_name =
                    build_array_type_name(base_name, inferred.array_dimensions);
    ...
    // 特定の関数名のパターンで型推論
    if (func_name == "get_array" ||
        func_name.find("array") != std::string::npos) {
    ...
    if (func_name == "create_counter" ||
        func_name.find("counter") != std::string::npos) {
...
InferredType
TypeInferenceEngine::infer_member_type(const InferredType &object_type,
    ...
    if (object_type.type_info == TYPE_STRUCT ||
        !object_type.type_name.empty()) {
        auto *type_manager = interpreter_.get_type_manager();
        std::string struct_name = object_type.type_name;
        if (struct_name.empty() && type_manager) {
            struct_name = type_manager->resolve_typedef(
                type_info_to_string(object_type.type_info));
        }
        if (type_manager && !struct_name.empty()) {
            ...
            const StructDefinition *struct_def =
                interpreter_.find_struct_definition(
                    strip_struct_prefix(resolved));
            if (!struct_def && resolved != struct_name) {
                struct_def = interpreter_.find_struct_definition(
                    strip_struct_prefix(struct_name));
            }

            if (struct_def) {
                if (const StructMember *member =
                    ...
                    InferredType inferred_member =
                        resolve_typedef_type(member_type_name);

                    ParsedTypeString alias_parsed =
                        parse_type_string(member_type_name);
                    ParsedTypeString inferred_parsed =
                        parse_type_string(inferred_member.type_name);

                    bool pointer_flag =
                        member->is_pointer || member->type == TYPE_POINTER ||
                        alias_parsed.is_pointer || inferred_parsed.is_pointer;
                    ...
                    if (base_name.empty() &&
                        member->pointer_base_type != TYPE_UNKNOWN) {
                        base_name =
                            type_info_to_string(member->pointer_base_type);
                    }
                    if (base_name.empty()) {
                    ...
                    if (pointer_flag) {
                        pointer_type_name =
                            base_name.empty() ? "void" : base_name;
                    ...
                    if (member->array_info.is_array()) {
                        ...
                        inferred_member.array_dimensions = static_cast<int>(
                            member->array_info.get_dimension_count());

                        std::string element_name =
                            pointer_flag ? pointer_type_name
                                         : inferred_member.type_name;
                        if (element_name.empty()) {
                            element_name = base_name;
                        }
                        inferred_member.type_name = build_array_type_name(
                            element_name, inferred_member.array_dimensions);

                        if (!pointer_flag) {
                            TypeInfo base_type =
                                member->array_info.base_type != TYPE_UNKNOWN
                                    ? member->array_info.base_type
                                    : inferred_member.type_info;
                            if (base_type >= TYPE_ARRAY_BASE) {
                                base_type = static_cast<TypeInfo>(
                                    base_type - TYPE_ARRAY_BASE);
                            }
                            if (base_type != TYPE_UNKNOWN) {
                    ...
                    if (inferred_member.type_info == TYPE_UNKNOWN) {
                        inferred_member.type_info =
                            pointer_flag ? TYPE_POINTER : member->type;
                    }

                    if (inferred_member.type_name.empty()) {
                        inferred_member.type_name =
                            pointer_flag ? pointer_type_name : member_type_name;
                    }
...
InferredType
TypeInferenceEngine::infer_array_element_type(const InferredType &array_type) {
    if (array_type.is_array && array_type.array_dimensions > 0) {
        ...
        if (!element_type.type_name.empty()) {
            element_type.type_name =
                remove_array_suffix(element_type.type_name, 1);
        }
        return element_type;
    }
    return InferredType();
...
InferredType TypeInferenceEngine::get_common_type(const InferredType &type1,
    ...
    if (is_numeric_type(type1.type_info) && is_numeric_type(type2.type_info)) {
        TypeInfo promoted =
            promote_numeric_type(type1.type_info, type2.type_info);
...
// typedef型の再帰的解決
InferredType
TypeInferenceEngine::resolve_typedef_type(const std::string &typedef_name) {
    auto *type_manager = interpreter_.get_type_manager();
    if (!type_manager) {
        return InferredType();
    }
    ...
    std::string base_string =
        parsed.base_type.empty() ? resolved : parsed.base_type;
    ...
    int effective_depth =
        pointer_depth > 0 ? pointer_depth : (is_pointer ? 1 : 0);
    if (is_pointer) {
        pointer_type_name =
            trim(pointer_type_name) + std::string(effective_depth, '*');
    }
    ...
    InferredType inferred(final_type_info, final_type_name, is_array,
                          dimensions);
    return inferred;
}

// 型エラーチェック（チェーン処理用）
bool TypeInferenceEngine::validate_chain_compatibility(
    const InferredType &object_type, const std::string &method_name,
    ...
    if (object_type.type_info == TYPE_INT ||
        object_type.type_info == TYPE_STRING ||
        object_type.type_info == TYPE_BOOL) {
...
// 型定義検索ヘルパー
const ASTNode *
TypeInferenceEngine::find_struct_definition(const std::string &struct_name) {
    // 簡易実装：とりあえずnullptrを返す
    return nullptr;
}

const ASTNode *
TypeInferenceEngine::find_union_definition(const std::string &union_name) {
    // 簡易実装：ユニオンサポートは後で実装
    return nullptr;
}

const ASTNode *
TypeInferenceEngine::find_typedef_definition(const std::string &typedef_name) {
