...
PointerMetadata PointerMetadata::create_array_element_pointer(Variable* array_var_param, size_t index, TypeInfo elem_type) {
    ...
    if (array_var_param && !array_var_param->array_values.empty() && 
        ...
        meta.address = reinterpret_cast<uintptr_t>(&array_var_param->array_values[index]);
    } else if (array_var_param && !array_var_param->multidim_array_values.empty() && 
               index < array_var_param->multidim_array_values.size()) {
        meta.address = reinterpret_cast<uintptr_t>(&array_var_param->multidim_array_values[index]);
    } else {
    ...
    // 範囲チェック用の情報を設定
    if (array_var_param && !array_var_param->array_values.empty()) {
        meta.array_start_addr = reinterpret_cast<uintptr_t>(&array_var_param->array_values[0]);
        meta.array_end_addr = meta.array_start_addr + (array_var_param->array_size * sizeof(int64_t));
    } else if (array_var_param && !array_var_param->multidim_array_values.empty()) {
        meta.array_start_addr = reinterpret_cast<uintptr_t>(&array_var_param->multidim_array_values[0]);
        meta.array_end_addr = meta.array_start_addr + (array_var_param->array_size * sizeof(int64_t));
    } else {
        // フォールバック：仮想アドレス
        uintptr_t base_addr = reinterpret_cast<uintptr_t>(array_var_param);
        meta.array_start_addr = base_addr;
        meta.array_end_addr = base_addr + (array_var_param->array_size * meta.type_size);
    }
