...
TypeParser::TypeParser(RecursiveParser *parser) : parser_(parser) {}
...
bool TypeParser::isValidType(const std::string &type_name) {
    ...
    if (base_type == "int" || base_type == "long" || base_type == "short" ||
        ...
        base_type == "char" || base_type == "void" || base_type == "float" ||
        base_type == "double" || base_type == "big" || base_type == "quad") {
        return true;
    }

    // 構造体定義のチェック
    if (parser_->struct_definitions_.find(base_type) !=
        parser_->struct_definitions_.end()) {
        return true;
    }

    // Enum定義のチェック
    if (parser_->enum_definitions_.find(base_type) !=
        parser_->enum_definitions_.end()) {
        return true;
    }

    // Interface定義のチェック
    if (parser_->interface_definitions_.find(base_type) !=
        parser_->interface_definitions_.end()) {
        return true;
    }

    // Union定義のチェック
    if (parser_->union_definitions_.find(base_type) !=
