...
private:
    ...
    ASTNode* parseTypedefTypeStatement(const std::string& type_name, bool isStatic, bool isConst);
    ASTNode* parseBasicTypeStatement(bool isStatic, bool isConst, bool isUnsigned);
    ASTNode* parseControlFlowStatement();
    ASTNode* parseExpressionOrAssignmentStatement();
    
    // 配列宣言と変数宣言のヘルパー
    ASTNode* parseArrayDeclaration(const std::string& base_type_name, const std::string& type_name,
                                  TypeInfo base_type_info, TypeInfo declared_type_info,
                                  bool isStatic, bool isConst, bool isUnsigned, bool is_reference);
    
    ASTNode* parseVariableDeclarationList(const std::string& first_var_name,
