...
ASTNode* StatementParser::parseStatement() {
    ...
    if (parser_->check(TokenType::TOK_IDENTIFIER)) {
        ...
        ASTNode* result = parseTypedefTypeStatement(type_name, isStatic, isConst);
...
// typedef型・構造体型・インターフェース型の変数宣言/関数定義
ASTNode* StatementParser::parseTypedefTypeStatement(const std::string& type_name, bool isStatic, bool isConst) {
    // typedef型か構造体型かをチェック
    bool is_typedef = parser_->typedef_map_.find(type_name) != parser_->typedef_map_.end();
    bool is_struct_type = parser_->struct_definitions_.find(type_name) != parser_->struct_definitions_.end();
    bool is_interface_type = parser_->interface_definitions_.find(type_name) != parser_->interface_definitions_.end();
    bool is_union_type = parser_->union_definitions_.find(type_name) != parser_->union_definitions_.end();
    bool is_enum_type = parser_->enum_definitions_.find(type_name) != parser_->enum_definitions_.end();
    
    if (!is_typedef && !is_struct_type && !is_interface_type && !is_union_type && !is_enum_type) {
    ...
    // ポインタ/参照修飾子をスキップ
    while (parser_->check(TokenType::TOK_MUL) || parser_->check(TokenType::TOK_BIT_AND)) {
        parser_->advance();
    }
    ...
    if (is_function) {
        ...
        parser_->consume(TokenType::TOK_SEMICOLON, "Expected ';' after enum variable declaration");
...
// 基本型の変数宣言/関数定義
ASTNode* StatementParser::parseBasicTypeStatement(bool isStatic, bool isConst, bool isUnsigned) {
    // 基本型のチェック
    ...
    if (parser_->check(TokenType::TOK_LBRACKET)) {
        return parseArrayDeclaration(base_type_name, type_name, base_type_info, declared_type_info, 
                                    isStatic, isConst, isUnsigned, is_reference);
    }
    
    // 関数宣言か通常の変数宣言かチェック
    if (parser_->check(TokenType::TOK_IDENTIFIER) || parser_->check(TokenType::TOK_MAIN)) {
        ...
        if (parser_->check(TokenType::TOK_LPAREN)) {
            ...
            return parseVariableDeclarationList(name_token.value, type_name, base_type_name, 
                                               base_type_info, declared_type_info,
                                               pointer_depth, isStatic, isConst, isUnsigned, is_reference);
        }
    } else {
...
// 配列宣言の処理（多次元対応）
ASTNode* StatementParser::parseArrayDeclaration(const std::string& base_type_name, const std::string& type_name,
                                                TypeInfo base_type_info, TypeInfo declared_type_info,
...
// 変数宣言リストの処理（カンマ区切り）
ASTNode* StatementParser::parseVariableDeclarationList(const std::string& first_var_name,
                                                        const std::string& type_name,
                                                        const std::string& base_type_name,
                                                        TypeInfo base_type_info,
                                                        TypeInfo declared_type_info,
                                                        int pointer_depth,
