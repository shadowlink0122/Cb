...
ASTNode* StructParser::parseUnionDeclaration() {
    parser_->error("C-style union declarations are not supported. Use typedef union syntax instead: typedef Name = Type1 | Type2;");
    return nullptr;
}
...
void StructParser::parseStructMembers(StructDefinition* struct_def) {
    ...
    while (!parser_->check(TokenType::TOK_RBRACE) && !parser_->isAtEnd()) {
        ...
        do {
            ...
            TypeInfo member_type_info = parser_->resolveParsedTypeInfo(var_parsed);
            ...
            if (member_base_type == struct_def->name && !var_parsed.is_pointer) {
                parser_->error("Self-recursive struct member '" + member_name + 
                      "' must be a pointer type. Use '" + struct_def->name + "* " + 
                      member_name + ";' instead of '" + struct_def->name + " " + 
                      member_name + ";'");
                return;
            }
            ...
            if (parser_->check(TokenType::TOK_LBRACKET)) {
                parser_->error("Old-style array declaration is not supported in struct members. Use 'int[2][2] member_name;' instead of 'int member_name[2][2];'");
                return;
            }
        ...
        parser_->consume(TokenType::TOK_SEMICOLON, "Expected ';' after struct member");
    }
}
...
void StructParser::parseUnionMembers(UnionDefinition* union_def) {
    parser_->error("C-style union member parsing is not supported. Union types use the typedef syntax: typedef Name = Value1 | Value2;");
}
...
void StructParser::detectCircularReference(
    ...
    if (parser_->detectCircularReference(struct_name, member_type, visited, path)) {
        std::string cycle_path = struct_name;
        for (size_t i = 1; i < path.size(); ++i) {
            cycle_path += " -> " + path[i];
        }
        parser_->error("Circular reference detected in struct value members: " + cycle_path + 
