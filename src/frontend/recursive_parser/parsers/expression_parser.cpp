...
ExpressionParser::ExpressionParser(RecursiveParser* parser) 
    : parser_(parser),
      primary_expression_parser_(std::make_unique<PrimaryExpressionParser>(parser)) {
}
...
ASTNode* ExpressionParser::parseAssignment() {
    ...
    if (parser_->check(TokenType::TOK_ASSIGN) || parser_->check(TokenType::TOK_PLUS_ASSIGN) || 
        ...
        if (left->node_type == ASTNodeType::AST_VARIABLE) {
            ...
            if (op_type != TokenType::TOK_ASSIGN) {
                parser_->error("Compound assignment to dereferenced pointer is not yet supported");
                return nullptr;
            }
            assign->left = std::unique_ptr<ASTNode>(left);
            assign->right = std::unique_ptr<ASTNode>(right);
