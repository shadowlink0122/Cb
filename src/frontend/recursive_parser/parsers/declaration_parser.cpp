...
ASTNode* DeclarationParser::parseFunctionDeclarationAfterName(
    ...
    if (!parser_->check(TokenType::TOK_RPAREN)) {
        do {
            ...
            // 型情報を設定（typedef解決を含む）
            std::string resolved_param_type = parser_->resolveTypedefChain(param_type);
            ...
            } else if (parser_->struct_definitions_.find(resolved_param_type) != parser_->struct_definitions_.end() ||
                       parser_->struct_definitions_.find(param_type) != parser_->struct_definitions_.end() ||
                       (resolved_param_type.substr(0, 7) == "struct " && 
                        parser_->struct_definitions_.find(resolved_param_type.substr(7)) != parser_->struct_definitions_.end())) {
                // struct型の場合（解決後の型名、元のtypedef名、または"struct Name"形式で検索）
                param->type_info = TYPE_STRUCT;
            } else if (parser_->enum_definitions_.find(resolved_param_type) != parser_->enum_definitions_.end() ||
                       parser_->enum_definitions_.find(param_type) != parser_->enum_definitions_.end()) {
                // enum型の場合  
                param->type_info = TYPE_ENUM;
            } else if (parser_->union_definitions_.find(resolved_param_type) != parser_->union_definitions_.end() ||
                       parser_->union_definitions_.find(param_type) != parser_->union_definitions_.end()) {
                // ユニオン型の場合
