...
UnionParser::UnionParser(RecursiveParser* parser) 
    : parser_(parser) {}
...
bool UnionParser::parseUnionValue(UnionDefinition& union_def) {
    if (parser_->check(TokenType::TOK_NUMBER)) {
        ...
        parser_->error("Invalid character literal: '" + char_str + "' (length: " + std::to_string(char_str.length()) + ")");
        return false;
        ...
        if (parser_->typedef_map_.find(type_name) != parser_->typedef_map_.end()) {
        ...
        if (parser_->struct_definitions_.find(type_name) != parser_->struct_definitions_.end()) {
        ...
        if (parser_->enum_definitions_.find(type_name) != parser_->enum_definitions_.end()) {
