...
void test_array_literal_success() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_success.cb", 
...
void test_array_literal_string_basic() {
    run_cb_test_with_output("../../tests/cases/array_literal/string_basic.cb", 
        [](const std::string& output, int exit_code) {
...
void test_array_literal_comprehensive() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_comprehensive.cb", 
        [](const std::string& output, int exit_code) {
...
void test_array_literal_empty() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_empty.cb", 
        [](const std::string& output, int exit_code) {
...
void test_array_literal_type_mismatch_int_string() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_fail_int_string.cb", 
        [](const std::string& output, int exit_code) {
...
void test_array_literal_type_mismatch_string_int() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_fail_string_int.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT(exit_code != 0, "Type mismatch test should fail");
            INTEGRATION_ASSERT(contains(output, "Error") || 
                             contains(output, "type expected") ||
...
void test_array_literal_type_mismatch_multiple() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_fail_multiple.cb", 
        [](const std::string& output, int exit_code) {
...
void test_array_literal_bounds() {
    run_cb_test_with_output("../../tests/cases/array_literal/test_array_bounds.cb", 
        [](const std::string& output, int exit_code) {
            // このテストは範囲外アクセステストなので、実装によって成功する場合もある
            // 出力があることを確認
            INTEGRATION_ASSERT(!output.empty(), "Should have some output");
        });
    integration_test_passed("array_literal_bounds");
}
