...
inline void test_integration_const_array() {
    ...
    // const配列要素への代入エラーテスト
    run_cb_test_with_output_and_time_auto(test_file_assign_error, 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "Expected error exit code for const array assignment");
            INTEGRATION_ASSERT_CONTAINS(output, "Cannot assign to const variable", "should contain const array error message");
        });
    integration_test_passed_with_time_auto("const array assign error test", test_file_assign_error);
    
    // const文字列配列要素への代入エラーテスト
    run_cb_test_with_output_and_time_auto(test_file_string_assign_error, 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "Expected error exit code for const string array assignment");
            INTEGRATION_ASSERT_CONTAINS(output, "Cannot assign to const variable", "should contain const array error message");
        });
    integration_test_passed_with_time_auto("const string array assign error test", test_file_string_assign_error);
    ...
    integration_test_passed_with_time_auto("const array read ok test", test_file_read_ok);
    
    // const多次元配列要素への代入エラーテスト
    run_cb_test_with_output_and_time_auto(test_file_multidim_error, 
        [](const std::string& output, int exit_code) {
    ...
    integration_test_passed_with_time_auto("const multidim array error test", test_file_multidim_error);
}
