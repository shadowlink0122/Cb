...
inline void test_integration_cross_type() {
    // 正常系：型変換が正しく動作する
    run_cb_test_with_output("../../tests/cases/cross_type/ok.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT(exit_code == 0, "Cross type ok should succeed");
        });
    integration_test_passed("cross_type ok test");

    // 異常系：型変換でエラーが発生する
    run_cb_test_with_output("../../tests/cases/cross_type/ng.cb", 
        [](const std::string& output, int exit_code) {
    ...
    integration_test_passed_with_error("cross_type ng test");
}
