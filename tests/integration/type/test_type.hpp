...
void test_integration_type() {
    ...
    run_cb_test_with_output("../cases/type/tiny/ng_assign.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "ng_assign.cb should fail due to range violation");
        });
    
    integration_test_passed_with_error("tiny type range violation (assignment)", "ng_assign.cb");
    
    // Test tiny type range violation with literal
    run_cb_test_with_output("../cases/type/tiny/ng_literal.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "ng_literal.cb should fail due to range violation");
        });
    
    integration_test_passed_with_error("tiny type range violation (literal)", "ng_literal.cb");
