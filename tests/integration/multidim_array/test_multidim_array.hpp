...
void test_multidim_array_literal_2d() {
    run_cb_test_with_output("../../tests/cases/multidim_array/literal_2d_test.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_EQ(0, exit_code, "2D array literal test should execute successfully");
        });
    integration_test_passed("test_multidim_array_literal_2d", "literal_2d_test.cb");
}

void test_multidim_array_literal_3d() {
    run_cb_test_with_output("../../tests/cases/multidim_array/literal_3d_test.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_EQ(0, exit_code, "3D array literal test should execute successfully");
        });
    integration_test_passed("test_multidim_array_literal_3d", "literal_3d_test.cb");
}

void test_multidim_array_assignment() {
    run_cb_test_with_output("../../tests/cases/multidim_array/assignment_test.cb", 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_EQ(0, exit_code, "Multidim array assignment test should execute successfully");
        });
    integration_test_passed("test_multidim_array_assignment", "assignment_test.cb");
}
