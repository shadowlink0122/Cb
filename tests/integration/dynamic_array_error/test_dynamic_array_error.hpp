...
inline void test_integration_dynamic_array_error() {
    ...
    // 単純な動的配列エラーテスト
    run_cb_test_with_output(test_simple, 
    ...
    integration_test_passed("dynamic_array_error simple test", test_simple);
    
    // 多次元動的配列エラーテスト
    run_cb_test_with_output(test_multidim, 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "Expected error exit code for multidim dynamic array test");
            INTEGRATION_ASSERT_CONTAINS(output, "Dynamic arrays are not supported yet", "should contain dynamic array error message");
        });
    integration_test_passed("dynamic_array_error multidim test", test_multidim);
    
    // 混合動的配列エラーテスト
    run_cb_test_with_output(test_mixed, 
        [](const std::string& output, int exit_code) {
            INTEGRATION_ASSERT_NE(0, exit_code, "Expected error exit code for mixed dynamic array test");
            INTEGRATION_ASSERT_CONTAINS(output, "Dynamic arrays are not supported yet", "should contain dynamic array error message");
        });
    integration_test_passed("dynamic_array_error mixed test", test_mixed);
}
