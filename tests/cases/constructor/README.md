...
2. ⏳ インタープリタ実装待ち
   - 構文: `Point p(10, 20);`
   - デフォルト引数: `Rectangle r(w = 100, h = 100);`
   - プライベート関数呼び出し: `self.initialize(px, py);`
...
2. **コンストラクタ内での`self`アクセス**
   - `self.member = value`: ✅ 動作
   - `self.prvtFunc()`: ✅ 対応済み（テスト準備完了）
