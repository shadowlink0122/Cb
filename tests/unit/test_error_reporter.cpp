...
void testExtractLine() {
    ...
    // Invalid line
    std::string line_invalid =
        SourceLocationUtils::extractLine(TEST_SOURCE, 100);
    assert(line_invalid == "");
...
void testExtractLinesWithContext() {
    ...
    auto lines =
        SourceLocationUtils::extractLinesWithContext(TEST_SOURCE, 3, 1);
...
void testFindSimilarStrings() {
    ...
    std::vector<std::string> candidates = {"unknown_var", "unknown_func",
                                           "known_var", "var", "variable"};

    // Find similar to "unknwn_var" (typo)
    auto similar =
        SourceLocationUtils::findSimilarStrings("unknwn_var", candidates, 2);
    assert(!similar.empty());
    assert(similar[0] == "unknown_var"); // Closest match

    // Find similar to "var"
    auto similar2 =
        SourceLocationUtils::findSimilarStrings("vars", candidates, 1);
...
void testErrorReporter() {
    ...
    reporter.report(ErrorSeverity::ERROR, loc,
                    "Undefined variable 'unknown_var'", suggestions);
...
void testFindSuggestions() {
    ...
    std::vector<std::string> known_names = {"variable", "value", "vector",
                                            "void", "volatile"};

    // Find suggestions for typo
    auto suggestions =
        ErrorReporter::findSuggestions("variabl", known_names, 3);
