...
### 優先度: 高
1. **エラーレポーターの統合** (次のタスク)
   - インタープリターの既存エラーハンドリングをErrorReporterに置き換え
   - 列番号をTokenに追加（現在は行番号のみ）
   - パーサーとレキサーでの列番号追跡
   - 実際のエラーメッセージで動作確認
...
### コード変更
- **変更ファイル数**: 4
- **追加行数**: 897行 (TypeHelpers: 182行, ErrorReporter: 715行)
- **削除行数**: 0行
- **純増**: +897行
