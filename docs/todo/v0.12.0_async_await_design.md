# Cb言語 v0.12.0 非同期処理設計書

**作成日**: 2025年10月26日  
**更新日**: 2025年11月7日  
**対象バージョン**: v0.12.0  
**ステータス**: 設計中

**前提**: v0.11.0でジェネリクス、デストラクタ、標準コレクション（Vector, Queue, Map）が完了

---

## 📋 目次

1. [設計方針と命名](#設計方針と命名)
2. [Cbにおける現状分析](#cbにおける現状分析)
3. [提案する実装アプローチ](#提案する実装アプローチ)
4. [構文と仕様](#構文と仕様)
5. [実装要件](#実装要件)
6. [段階的実装計画](#段階的実装計画)

---

## 🎯 設計方針と命名

### 命名の選択肢比較

| アプローチ | 長所 | 短所 | 言語との整合性 |
|----------|------|------|--------------|
| **Coroutine** | ・C++20のcoroutineと一貫性<br>・低レベルで柔軟性が高い<br>・「Cっぽい」印象 | ・使い勝手が複雑<br>・学習コストが高い<br>・ボイラープレートが多い | C++との親和性: ⭐⭐⭐⭐⭐ |
| **async/await** | ・直感的で使いやすい<br>・TypeScript/Rust風<br>・モダンな印象 | ・C言語との乖離<br>・予約語が2つ増える | TypeScript/Rustとの親和性: ⭐⭐⭐⭐⭐ |
| **ハイブリッド** | ・両方のメリット<br>・段階的な実装が可能 | ・概念の混在<br>・仕様が複雑化 | バランス: ⭐⭐⭐ |

### 🎖️ 推奨: **async/await + 内部でcoroutine実装**

**理由**:
1. **ユーザビリティ優先**: Cb言語は「実用性重視」の設計思想
2. **既存言語との整合性**: TypeScriptやRustの知識が活かせる
3. **学習コストの低減**: 直感的な構文で非同期処理を記述
4. **段階的実装**: Phase 1で基本的なasync/await、将来的にcoroutineの詳細制御も追加可能

**実装戦略**:
```
ユーザー向けインターフェース: async/await（高レベル）
         ↓
内部実装: Coroutine機構（低レベル）
         ↓
実行基盤: Event Loop / Executor
```

---

## 🔍 Cbにおける現状分析

### 既存の機能

#### ✅ 利用可能な機能

1. **関数ポインタ**: コールバック機構として利用可能
   ```cb
   int* callback = int func(int x) { return x * 2; };
   ```

2. **Lambda式**: 匿名関数として非同期タスクを定義可能
   ```cb
   int* task = int func() {
       // 非同期処理
       return result;
   };
   ```

3. **構造体**: Future/Promiseの状態管理に利用
   ```cb
   struct Future<T> {
       T value;
       bool is_ready;
   };
   ```

4. **Interface/Impl**: 非同期トレイトの定義に活用
   ```cb
   interface AsyncTask {
       void run();
       bool is_complete();
   };
   ```

5. **スコープベースのリソース管理**: defer、デストラクタによるクリーンアップ
   ```cb
   {
       Resource r;
       defer cleanup();
   }  // 自動クリーンアップ
   ```

#### ❌ 不足している機能

1. **スレッド/並行実行機構**
   - 現状: インタプリタはシングルスレッド実行
   - 必要: マルチスレッドのサポート、またはEvent Loop

2. **イベントループ**
   - 現状: 逐次実行のみ
   - 必要: 非同期タスクのスケジューリング

3. **スリープ/待機機構**
   - 現状: ブロッキング操作のみ
   - 必要: ノンブロッキングな待機

4. **チャネル/通信機構**
   - 現状: なし
   - 必要: 非同期タスク間の通信

5. **エラーハンドリング拡張**
   - 現状: 基本的なthrowのみ
   - 必要: async関数内でのエラー伝播

6. **タイマー/時間管理**
   - 現状: 基本的な時間関数のみ
   - 必要: タイムアウト、遅延実行

### 非同期処理に必要な追加機能

#### 🔴 最優先（v0.11.0で実装必須）

1. **Event Loop実装**
   - タスクキューの管理
   - スケジューラー
   - 実行コンテキストの切り替え

2. **Future/Promise型**
   - 非同期結果の表現
   - 状態管理（Pending, Ready, Error）

3. **async/await構文**
   - パーサー拡張
   - コードジェネレーション

#### 🟡 高優先（v0.11.x-v0.12.0）

4. **タイマー機能**
   - `sleep_async(duration)`
   - `timeout(duration, task)`

5. **基本的なチャネル**
   - Producer-Consumer パターン
   - 非同期メッセージング

6. **スレッドプール（オプション）**
   - 並列実行のサポート
   - CPUバウンドタスクの処理

#### 🟢 中優先（v0.12.0以降）

7. **インラインアセンブリ（ASM）**
   - 低レベル最適化
   - アトミック操作
   - 現時点では**不要**（Event Loop実装で十分）

8. **Select/Race構文**
   - 複数のFutureを同時に待機
   - タイムアウト処理

---

## 💡 提案する実装アプローチ

### Phase 1: 基本的なasync/await（v0.11.0目標）

**スコープ**: 協調的マルチタスキング（Cooperative Multitasking）

**特徴**:
- Event Loop ベース
- シングルスレッド実行
- 明示的な`await`ポイントでタスク切り替え
- I/Oバウンドタスクに最適

**利点**:
- ✅ 実装が比較的シンプル
- ✅ デッドロックの心配が少ない
- ✅ デバッグが容易
- ✅ JavaScript/TypeScriptと同様のモデル

**制限**:
- ⚠️ CPUバウンドタスクには不向き（ブロッキング）
- ⚠️ 真の並列実行は不可

### Phase 2: スレッドプール統合（v0.12.0以降）

**スコープ**: プリエンプティブマルチタスキング（Preemptive Multitasking）

**特徴**:
- スレッドプールでCPUバウンドタスクを実行
- async/await構文はそのまま
- 内部でスレッドを管理

---

## 📝 構文と仕様

### 基本構文

#### async関数の定義

```cb
// 基本的なasync関数
async int fetch_data(string url) {
    println("Fetching data from: ", url);
    
    // 非同期待機（ここでタスク切り替え）
    string result = await http_get(url);
    
    return parse_int(result);
}

// void戻り値のasync関数
async void log_async(string message) {
    await sleep_ms(100);
    println(message);
}
```

#### awaitの使用

```cb
void main() {
    // async関数を呼び出すとFutureが返る
    Future<int> future = fetch_data("https://api.example.com/data");
    
    // awaitで結果を待つ（ブロッキング）
    int data = await future;
    println("Data: ", data);
}
```

#### 複数のawait

```cb
async int process_multiple() {
    // 順次実行
    int data1 = await fetch_data("url1");
    int data2 = await fetch_data("url2");
    int data3 = await fetch_data("url3");
    
    return data1 + data2 + data3;
}
```

#### 並行実行（Future配列）

```cb
async int process_parallel() {
    // 複数のタスクを同時に開始
    Future<int> f1 = fetch_data("url1");
    Future<int> f2 = fetch_data("url2");
    Future<int> f3 = fetch_data("url3");
    
    // すべてのタスクが完了するまで待つ
    int data1 = await f1;
    int data2 = await f2;
    int data3 = await f3;
    
    return data1 + data2 + data3;
}
```

### 型システム統合

#### Future型（Result統合版）

```cb
// Futureは成功時の値のみを持つ（シンプル版）
struct Future<T> {
    T value;              // 結果の値
    bool is_ready;        // 完了フラグ
};

// FutureとResultの組み合わせが推奨パターン
// async関数はFuture<Result<T, E>>を返す
async Result<int, NetworkError> fetch_data(string url) {
    // ...
}

// 使用例
async void main() {
    // Futureを取得
    Future<Result<int, NetworkError>> future = fetch_data("https://api.example.com");
    
    // awaitでResultを取得
    Result<int, NetworkError> result = await future;
    
    // パターンマッチでエラーハンドリング
    match (result) {
        Ok(value) => println("Success: ", value),
        Err(error) => println("Error: ", error.message)
    }
}

// または?オペレーターで簡潔に
async Result<int, Error> process() {
    int data = await fetch_data("url")?;  // エラーは自動伝播
    return Ok(data * 2);
}
```

#### Futureの状態確認（ノンブロッキング）

```cb
// Futureの完了状態を確認
bool is_ready<T>(Future<T> future);

// ポーリングパターン（非推奨、awaitを使用すべき）
async void polling_example() {
    Future<Result<int, Error>> future = fetch_data("url");
    
    while (!is_ready(future)) {
        println("Waiting...");
        await sleep_ms(100);
    }
    
    Result<int, Error> result = await future;
    // ...
}
```

#### async関数の型

```cb
// async関数の型は Future<T> を返す関数ポインタ
typedef Future<int> AsyncIntFunc = async int func(string url);

async int my_async_func(string url) {
    return await fetch_data(url);
}

void main() {
    AsyncIntFunc f = my_async_func;
    Future<int> future = f("https://example.com");
    int result = await future;
}
```

### エラーハンドリング（Rust風Result型）

#### Result型の定義

```cb
// Result型: 成功（Ok）またはエラー（Err）を表現
enum Result<T, E> {
    Ok(T),      // 成功した場合の値
    Err(E)      // エラーが発生した場合の値
};

// よく使うエラー型
struct Error {
    string message;
    int code;
};

// 特定のエラー型
struct NetworkError {
    string message;
    int status_code;
};

struct FileError {
    string message;
    string path;
};
```

#### async関数でResult型を返す

```cb
// Result<T, E>を返すasync関数
async Result<int, NetworkError> fetch_data(string url) {
    if (url == "") {
        NetworkError err;
        err.message = "URL cannot be empty";
        err.status_code = 400;
        return Err(err);
    }
    
    string response = await http_get(url);
    
    if (response == "") {
        NetworkError err;
        err.message = "Empty response";
        err.status_code = 500;
        return Err(err);
    }
    
    int value = parse_int(response);
    return Ok(value);
}
```

#### Result型のパターンマッチング

```cb
async void main() {
    Result<int, NetworkError> result = await fetch_data("https://api.example.com");
    
    // パターンマッチでエラーハンドリング
    match (result) {
        Ok(value) => {
            println("Success: ", value);
        },
        Err(error) => {
            println("Error: ", error.message);
            println("Status: ", error.status_code);
        }
    }
}
```

#### `?`オペレーター（エラー伝播）

```cb
// Rustの`?`オペレーターと同様の機能
async Result<int, Error> process_data(string url) {
    // エラーが発生した場合、自動的に返す
    int data = await fetch_data(url)?;  // Errの場合はここで即return
    
    int processed = data * 2;
    return Ok(processed);
}

// 複数のエラーチェック
async Result<int, Error> complex_operation() {
    int data1 = await fetch_data("url1")?;
    int data2 = await fetch_data("url2")?;
    int data3 = await fetch_data("url3")?;
    
    return Ok(data1 + data2 + data3);
}
```

#### unwrap/expect（開発時の簡易的なエラーハンドリング）

```cb
async void main() {
    Result<int, NetworkError> result = await fetch_data("https://api.example.com");
    
    // unwrap: エラーの場合はパニック（プログラム終了）
    int value = result.unwrap();  // 開発時のみ推奨
    
    // expect: カスタムエラーメッセージ付きでパニック
    int value2 = result.expect("Failed to fetch data from API");
}
```

#### unwrap_or/unwrap_or_else（デフォルト値の提供）

```cb
async void main() {
    Result<int, NetworkError> result = await fetch_data("https://api.example.com");
    
    // unwrap_or: エラーの場合はデフォルト値を返す
    int value = result.unwrap_or(0);
    println("Value: ", value);
    
    // unwrap_or_else: エラーの場合は関数を実行
    int value2 = result.unwrap_or_else(int func(NetworkError e) {
        println("Error occurred: ", e.message);
        return -1;
    });
}
```

#### map/and_then（Result型の変換）

```cb
// map: Ok値を変換（エラーはそのまま伝播）
async Result<string, NetworkError> fetch_and_format(string url) {
    Result<int, NetworkError> result = await fetch_data(url);
    
    // Okの場合のみ値を変換
    return result.map(int func(int value) {
        return "Value is: " + to_string(value);
    });
}

// and_then: Result型を返す関数でチェーン
async Result<int, NetworkError> fetch_and_double(string url) {
    Result<int, NetworkError> result = await fetch_data(url);
    
    return result.and_then(Result<int, NetworkError> func(int value) {
        if (value < 0) {
            NetworkError err;
            err.message = "Negative value not allowed";
            err.status_code = 400;
            return Err(err);
        }
        return Ok(value * 2);
    });
}
```

#### 従来のtry-catchとの共存

```cb
// try-catchも引き続きサポート（緊急時のみ使用を推奨）
async void legacy_error_handling() {
    try {
        // パニックを起こす可能性のあるコード
        int value = some_result.unwrap();
        println("Value: ", value);
    } catch (PanicError& e) {
        println("Panic caught: ", e.message);
    }
}
```

#### 実用例: 複数のエラー型を扱う

```cb
// 複数のエラー型をUnion型でまとめる
typedef AppError = NetworkError | FileError | ParseError;

async Result<User, AppError> load_user(int user_id) {
    // ファイルからユーザー設定を読み込む
    Result<string, FileError> config_result = await read_file("config.json");
    string config = config_result?;  // エラーは自動的に伝播
    
    // APIからユーザー情報を取得
    Result<string, NetworkError> api_result = await http_get("/users/" + to_string(user_id));
    string user_json = api_result?;
    
    // JSONをパース
    Result<User, ParseError> parse_result = parse_user(user_json);
    User user = parse_result?;
    
    return Ok(user);
}

async void main() {
    Result<User, AppError> result = await load_user(123);
    
    match (result) {
        Ok(user) => {
            println("User loaded: ", user.name);
        },
        Err(error) => {
            match (error) {
                NetworkError(e) => println("Network error: ", e.message),
                FileError(e) => println("File error: ", e.path),
                ParseError(e) => println("Parse error: ", e.message)
            }
        }
    }
}
```

### タイマーとスリープ

```cb
// ミリ秒単位のスリープ（非同期）
async void sleep_ms(int milliseconds);

// 秒単位のスリープ（非同期）
async void sleep(int seconds) {
    await sleep_ms(seconds * 1000);
}

// タイムアウト付き実行
async int with_timeout(Future<int> future, int timeout_ms) {
    // 将来的な実装（v0.12.0以降）
    // ...
}
```

### 実用例

#### HTTPリクエスト（Result型版）

```cb
async Result<string, NetworkError> http_get(string url) {
    println("GET ", url);
    await sleep_ms(100);  // ネットワーク遅延シミュレート
    
    if (url == "") {
        NetworkError err;
        err.message = "Empty URL";
        err.status_code = 400;
        return Err(err);
    }
    
    return Ok("Response from " + url);
}

async Result<void, NetworkError> main() {
    Result<string, NetworkError> result = await http_get("https://api.example.com");
    
    match (result) {
        Ok(response) => println("Response: ", response),
        Err(error) => println("Error: ", error.message)
    }
    
    return Ok(void);  // void戻り値のOk
}

// または?オペレーターで簡潔に
async Result<void, NetworkError> main_short() {
    string response = await http_get("https://api.example.com")?;
    println("Response: ", response);
    return Ok(void);
}
```

#### ファイル読み込み（Result型版）

```cb
async Result<string, FileError> read_file_async(string path) {
    println("Reading file: ", path);
    await sleep_ms(50);  // I/O待機シミュレート
    
    if (!file_exists(path)) {
        FileError err;
        err.message = "File not found";
        err.path = path;
        return Err(err);
    }
    
    return Ok("File contents from " + path);
}

async Result<void, FileError> process_files() {
    // 並行読み込み
    Future<Result<string, FileError>> f1 = read_file_async("file1.txt");
    Future<Result<string, FileError>> f2 = read_file_async("file2.txt");
    Future<Result<string, FileError>> f3 = read_file_async("file3.txt");
    
    // すべての結果を取得（?で簡潔に）
    string content1 = await f1?;
    string content2 = await f2?;
    string content3 = await f3?;
    
    println("All files loaded!");
    return Ok(void);
}
```

#### データベースクエリ（Result型版）

```cb
struct User {
    int id;
    string name;
    string email;
};

struct DbError {
    string message;
    string query;
};

async Result<User, DbError> fetch_user(int user_id) {
    println("Fetching user ", user_id);
    await sleep_ms(200);  // DB遅延
    
    if (user_id <= 0) {
        DbError err;
        err.message = "Invalid user ID";
        err.query = "SELECT * FROM users WHERE id = " + to_string(user_id);
        return Err(err);
    }
    
    User user;
    user.id = user_id;
    user.name = "Alice";
    user.email = "alice@example.com";
    return Ok(user);
}

async Result<void, DbError> main() {
    Result<User, DbError> result = await fetch_user(1);
    
    match (result) {
        Ok(user) => {
            println("User: ", user.name, " (", user.email, ")");
        },
        Err(error) => {
            println("Database error: ", error.message);
            println("Query: ", error.query);
        }
    }
    
    return Ok(void);
}

// または?とunwrap_orを組み合わせて
async void main_with_defaults() {
    Result<User, DbError> result = await fetch_user(1);
    User user = result.unwrap_or(User{0, "Guest", "guest@example.com"});
    println("User: ", user.name);
}
```

---

## 🔧 実装要件

### 1. パーサー拡張

#### 新しいトークン

```cpp
enum class TokenType {
    // 既存のトークン...
    
    // 非同期処理用の新しいトークン
    TOK_ASYNC,      // "async"
    TOK_AWAIT,      // "await"
    TOK_FUTURE,     // "Future" (組み込み型として)
    
    // Result型用
    TOK_RESULT,     // "Result" (組み込み型として)
    TOK_OK,         // "Ok"
    TOK_ERR,        // "Err"
    TOK_QUESTION,   // "?" (エラー伝播オペレーター)
};
```

#### AST拡張

```cpp
// 新しいASTノードタイプ
enum class ASTNodeType {
    // 既存のノード...
    
    // 非同期処理用
    AST_ASYNC_FUNCTION,     // async関数定義
    AST_AWAIT_EXPRESSION,   // await式
    AST_FUTURE_TYPE,        // Future型
    
    // Result型用
    AST_RESULT_TYPE,        // Result型
    AST_OK_EXPRESSION,      // Ok(value)
    AST_ERR_EXPRESSION,     // Err(error)
    AST_TRY_OPERATOR,       // ? オペレーター
};

// async関数のフラグ
struct ASTNode {
    // 既存のフィールド...
    
    bool is_async = false;  // この関数がasyncかどうか
};
```

#### BNF拡張

```bnf
<async_function> ::= 'async' <type_specifier> '(' <parameter_list>? ')' <block>

<await_expression> ::= 'await' <expression>

<future_type> ::= 'Future' '<' <type_specifier> '>'

<result_type> ::= 'Result' '<' <type_specifier> ',' <type_specifier> '>'

<ok_expression> ::= 'Ok' '(' <expression> ')'

<err_expression> ::= 'Err' '(' <expression> ')'

<try_operator> ::= <expression> '?'
```

### 2. インタプリタ拡張

#### Event Loop実装

```cpp
class EventLoop {
public:
    void schedule_task(std::function<void()> task);
    void run();
    void stop();
    
private:
    std::queue<std::function<void()>> task_queue_;
    bool is_running_ = false;
};
```

#### Result型実装

```cpp
// Result型のバリアント（Ok/Err）
enum class ResultVariant {
    OK,
    ERR
};

template<typename T, typename E>
class Result {
public:
    // コンストラクタ
    static Result<T, E> Ok(T value);
    static Result<T, E> Err(E error);
    
    // 状態確認
    bool is_ok() const;
    bool is_err() const;
    
    // 値の取得
    T unwrap();  // Errの場合はパニック
    T expect(const std::string& message);  // カスタムメッセージ付きパニック
    T unwrap_or(T default_value);
    T unwrap_or_else(std::function<T(E)> func);
    
    // 変換
    template<typename U>
    Result<U, E> map(std::function<U(T)> func);
    
    template<typename U>
    Result<U, E> and_then(std::function<Result<U, E>(T)> func);
    
private:
    ResultVariant variant_;
    std::optional<T> ok_value_;
    std::optional<E> err_value_;
};
```

#### Future実装（シンプル版）

```cpp
template<typename T>
class Future {
public:
    bool is_ready() const;
    T get();  // ブロッキング
    void set_value(T value);
    
private:
    std::optional<T> value_;
    bool is_ready_ = false;
};

// FutureはResult型と組み合わせて使用
// Future<Result<T, E>> が推奨パターン
```

#### async関数の実行

```cpp
class Interpreter {
public:
    // 既存のメソッド...
    
    // async関数の実行
    int64_t execute_async_function(const ASTNode* node);
    
    // await式の評価
    int64_t evaluate_await_expression(const ASTNode* node);
    
    // ?オペレーター（try operator）の評価
    int64_t evaluate_try_operator(const ASTNode* node);
    
private:
    EventLoop event_loop_;
    std::unordered_map<std::string, std::shared_ptr<FutureBase>> futures_;
    std::unordered_map<std::string, std::shared_ptr<ResultBase>> results_;
};
```

### 3. 標準ライブラリ拡張

#### stdlib/async.cb

```cb
// Result型の定義
enum Result<T, E> {
    Ok(T),
    Err(E)
};

// Result型のメソッド（組み込み）
bool is_ok<T, E>(Result<T, E> result);
bool is_err<T, E>(Result<T, E> result);
T unwrap<T, E>(Result<T, E> result);
T expect<T, E>(Result<T, E> result, string message);
T unwrap_or<T, E>(Result<T, E> result, T default_value);

// Future型の定義
struct Future<T> {
    T value;
    bool is_ready;
};

// タイマー関数
async void sleep_ms(int milliseconds);
async void sleep(int seconds);

// Future操作
bool is_ready<T>(Future<T> future);
```

#### stdlib/concurrent.cb（将来的）

```cb
// スレッド操作
struct Thread {
    int id;
    bool is_running;
};

Thread spawn_thread(void* func);
void join_thread(Thread thread);

// チャネル
struct Channel<T> {
    int capacity;
};

Channel<T> create_channel<T>(int capacity);
async void send<T>(Channel<T> chan, T value);
async T receive<T>(Channel<T> chan);
```

---

## 🚀 段階的実装計画

### ⚠️ 重要: 実装順序の依存関係

async/awaitとResult型の実装には、以下の機能が前提条件となります：

```
Phase 0: ジェネリクス（テンプレート）
    ↓
Phase 1: Event Loop + タイマー
    ↓
Phase 2: Future<T> 型
    ↓
Phase 3: Result<T, E> 型 + パターンマッチング
    ↓
Phase 4: async/await構文
    ↓
Phase 5: ?オペレーター
```

### Phase 0: ジェネリクス実装（前提条件）（4週間）

**目標**: `Future<T>`と`Result<T, E>`を実装するための基盤

#### なぜジェネリクスが必要か

async/awaitで必須の型：
```cb
// これらはすべてジェネリクスが必要
Future<int>
Future<string>
Future<User>
Future<Result<int, Error>>

Result<int, NetworkError>
Result<User, DbError>
Result<void, FileError>
```

#### タスク（v0.11.0_generics_spec.mdから）

1. **基本的なジェネリック構造体**（2週間）
   - [ ] 構文解析: `struct Box<T> { T value; }`
   - [ ] 型パラメータ管理
   - [ ] インスタンス化とキャッシュ
   - [ ] テスト: 基本的な構造体

2. **ジェネリック関数**（1週間）
   - [ ] 構文解析: `T max<T>(T a, T b)`
   - [ ] 関数テンプレートの実装
   - [ ] テスト: 基本的な関数

3. **Enum型のジェネリクス対応**（1週間）
   - [ ] 構文解析: `enum Result<T, E> { Ok(T), Err(E) }`
   - [ ] バリアント付きEnum
   - [ ] パターンマッチング統合
   - [ ] テスト: Result型のプロトタイプ

#### 最小限の実装範囲

async/await実装に必要な機能のみに絞る：
- ✅ ジェネリック構造体（`Future<T>`用）
- ✅ ジェネリックEnum（`Result<T, E>`用）
- ✅ 基本的な型パラメータ（`<T>`, `<T, E>`）
- ⚠️ 型制約は後回し（v0.11.1以降）
- ⚠️ ジェネリックInterfaceは後回し（v0.11.1以降）
- ⚠️ 型推論は後回し（v0.12.0以降）

#### 実装順序の理由

1. **Future<T>の実装に必須**
   ```cb
   struct Future<T> {
       T value;
       bool is_ready;
   };
   ```

2. **Result<T, E>の実装に必須**
   ```cb
   enum Result<T, E> {
       Ok(T),
       Err(E)
   };
   ```

3. **async関数の戻り値に必須**
   ```cb
   async Result<int, Error> fetch_data(string url) {
       // ...
   }
   // ↓ 内部的には
   Future<Result<int, Error>> fetch_data(string url);
   ```

### Phase 1: Event Loop + タイマー（2週間）

**目標**: 非同期タスクの実行基盤（ジェネリクス不要の部分）

#### タスク
1. **Event Loop実装**
   - [ ] タスクキューの実装
   - [ ] スケジューラーの実装
   - [ ] run/stop メソッド

2. **タイマー実装**
   - [ ] sleep_ms の実装（まずは非ジェネリック版）
   - [ ] タイマーキューの管理

#### テスト
- Event Loopの基本動作
- タイマーの精度
- タスクスケジューリング

### Phase 2: Future<T>型（1週間）

**目標**: ジェネリックなFuture型の実装（Phase 0の成果を利用）

#### タスク
1. **Future<T>構造体の定義**
   - [ ] ジェネリック構造体として実装
   - [ ] 状態管理（Pending, Ready）
   - [ ] get/set メソッド

2. **Future<T>の基本操作**
   - [ ] `is_ready<T>(Future<T>)` 関数
   - [ ] Future変数の宣言と初期化

#### テスト
- Future<int>, Future<string> の動作確認
- 状態遷移のテスト
- 複数のFutureの管理

### Phase 3: Result<T, E>型 + パターンマッチング（3週間）

**目標**: Rust風Result型の完全実装（Phase 0の成果を利用）

#### タスク
1. **Result<T, E> Enum定義**（1週間）
   - [ ] `enum Result<T, E>` の構文解析
   - [ ] Ok/Err バリアントの実装
   - [ ] バリアントデータの管理

2. **パターンマッチング実装**（1週間）
   - [ ] `match` 文の構文解析
   - [ ] Enumバリアントのマッチング
   - [ ] 値の抽出（`Ok(value)`, `Err(error)`）

3. **Result型のメソッド**（1週間）
   - [ ] `unwrap()` / `expect()`
   - [ ] `unwrap_or()` / `unwrap_or_else()`
   - [ ] `is_ok()` / `is_err()`
   - [ ] `map()` / `and_then()`（ラムダ式を利用）

#### テスト
- Result型の基本動作
- パターンマッチングの動作確認
- Result型メソッドのテスト
- エラー伝播のテスト

### Phase 4: async/await構文（3週間）

**目標**: async/await構文の実装（Phase 0-3の成果を利用）

#### タスク
1. **パーサー拡張**（1週間）
   - [ ] `async` キーワードの認識
   - [ ] `await` キーワードの認識
   - [ ] async関数定義のパース
   - [ ] await式のパース

2. **AST拡張**（1週間）
   - [ ] AST_ASYNC_FUNCTION ノード
   - [ ] AST_AWAIT_EXPRESSION ノード
   - [ ] is_async フラグ

3. **インタプリタ実装**（1週間）
   - [ ] async関数の実行（Future<Result<T, E>>を返す）
   - [ ] await式の評価
   - [ ] コンテキスト切り替え
   - [ ] Event Loopとの統合

#### テスト
- 基本的なasync関数の定義と呼び出し
- await式の動作
- 複数のawaitの連続実行
- async関数からのResult型返却

### Phase 5: ?オペレーター（1週間）

**目標**: エラー伝播の簡潔な記法

#### タスク
1. **?オペレーターの構文解析**
   - [ ] `expr?` の認識
   - [ ] 後置演算子としての実装

2. **エラー伝播ロジック**
   - [ ] Result型のチェック
   - [ ] Errの場合の早期リターン
   - [ ] Okの場合の値の抽出

3. **型チェック**
   - [ ] 関数の戻り値がResult型であることを確認
   - [ ] エラー型の互換性チェック

#### テスト
- ?オペレーターの基本動作
- エラー伝播のテスト
- ネストした?オペレーター
- async関数内での?オペレーター

### Phase 6: 並行実行とライブラリ拡充（2週間）

**目標**: 実用的な非同期プログラミング

#### タスク
1. **並行実行のサポート**
   - [ ] 複数のFutureを同時に開始
   - [ ] 各Futureを個別にawait

2. **標準ライブラリ**
   - [ ] stdlib/async.cb の充実
   - [ ] 基本的なエラー型の定義
   - [ ] ユーティリティ関数

#### テスト
- 並行実行の動作確認
- タスク完了順序のテスト
- 実用的なサンプルコード

---

## 📊 実装優先度マトリックス

| 機能 | 優先度 | 実装難易度 | 必要性 | Phase |
|-----|-------|----------|-------|-------|
| **ジェネリクス（基本）** | 🔴 最優先 | ⭐⭐⭐⭐⭐ | **前提条件** | 0 |
| Event Loop | 🔴 最優先 | ⭐⭐⭐⭐ | 必須 | 1 |
| Future<T>型 | 🔴 最優先 | ⭐⭐⭐ | 必須 | 2 |
| Result<T, E>型 | 🔴 最優先 | ⭐⭐⭐⭐ | 必須 | 3 |
| **パターンマッチング** | 🔴 最優先 | ⭐⭐⭐⭐ | 必須 | 3 |
| async/await構文 | 🔴 最優先 | ⭐⭐⭐⭐⭐ | 必須 | 4 |
| ?オペレーター | 🔴 最優先 | ⭐⭐⭐ | 必須 | 5 |
| タイマー | 🟡 高 | ⭐⭐ | 必須 | 1 |
| 並行実行 | 🟡 高 | ⭐⭐⭐ | 重要 | 6 |
| チャネル | � 中 | ⭐⭐⭐⭐ | 便利 | v0.12.0 |
| スレッドプール | 🟢 中 | ⭐⭐⭐⭐⭐ | 便利 | v0.12.0 |
| 型制約（ジェネリクス） | 🟢 中 | ⭐⭐⭐ | 便利 | v0.11.1 |
| インラインASM | 🔵 低 | ⭐⭐⭐⭐⭐ | 不要 | v0.13.0 |

---

## 🎓 参考実装

### JavaScript/TypeScript
- **Event Loop**: Node.jsのイベントループモデル
- **構文**: async/await構文
- **Promise**: Futureの実装モデル

### Rust
- **Future trait**: Futureの型システム
- **Executor**: タスク実行機構
- **Pin**: メモリ安全性

### C++20 Coroutines
- **co_await**: await式のセマンティクス
- **Promise type**: Futureの実装
- **Coroutine Handle**: 実行制御

### Python asyncio
- **Event Loop**: イベントループの実装
- **Task**: タスク管理
- **await式**: 構文とセマンティクス

---

## ❓ よくある質問

### Q1: なぜインラインアセンブリは不要？

**A**: 現時点では以下の理由から優先度が低い：
- Event Loopベースの実装で十分
- アトミック操作は標準ライブラリで提供可能
- 低レベル最適化は将来的にLLVMバックエンドで対応
- 開発リソースをコア機能に集中すべき

### Q2: スレッドプールはv0.11.0に含まれない？

**A**: Phase 1ではEvent Loopのみ実装する理由：
- シングルスレッド実装の方がシンプル
- I/Oバウンドタスクには十分
- デバッグとテストが容易
- v0.12.0でスレッドプールを追加する計画

### Q3: async関数はどのように実行される？

**A**: 以下の流れで実行：
1. async関数呼び出し → Futureを即座に返す
2. Futureをawait → Event Loopでタスクをスケジュール
3. await地点でコンテキスト切り替え → 他のタスクを実行
4. タスク完了 → Futureに結果を設定
5. 呼び出し元に制御を戻す

### Q6: ジェネリクスの実装規模はどのくらい？

**A**: async/awaitに必要な最小限の機能に絞ります：

**v0.11.0で実装するジェネリクス機能**：
- ✅ ジェネリック構造体（`struct Future<T>`, `struct Box<T>`）
- ✅ ジェネリックEnum（`enum Result<T, E>`）
- ✅ 複数の型パラメータ（`<T>`, `<T, E>`）
- ✅ 基本的なインスタンス化（`Future<int>`, `Result<User, Error>`）

**v0.11.1以降に延期する機能**：
- ⚠️ 型制約（`<T: int | long>`）
- ⚠️ ジェネリック関数（`T max<T>(T a, T b)`）- 必要最小限のみ
- ⚠️ ジェネリックInterface（`interface Container<T>`）
- ⚠️ impl統合（`impl Trait<T> for Struct<T>`）

**実装時間の見積もり**：
- ジェネリック構造体: 2週間
- ジェネリックEnum: 1週間
- テストとデバッグ: 1週間
- **合計**: 4週間

**参考**: v0.11.0_generics_spec.mdには完全なジェネリクス仕様が記載されていますが、async/await実装のために段階的に進めます。

### Q7: v0.11.0のリリース時期は？

**A**: 実装規模を考慮した見積もり：

| Phase | 機能 | 期間 | 完了予定 |
|-------|-----|------|---------|
| Phase 0 | ジェネリクス（基本） | 4週間 | 2025年11月23日 |
| Phase 1 | Event Loop + タイマー | 2週間 | 2025年12月7日 |
| Phase 2 | Future<T>型 | 1週間 | 2025年12月14日 |
| Phase 3 | Result<T, E> + パターンマッチング | 3週間 | 2026年1月4日 |
| Phase 4 | async/await構文 | 3週間 | 2026年1月25日 |
| Phase 5 | ?オペレーター | 1週間 | 2026年2月1日 |
| Phase 6 | 並行実行 + ライブラリ | 2週間 | 2026年2月15日 |
| **合計** | | **16週間** | **2026年2月中旬** |

**リリース目標**: 2026年2月中旬

**注**: これはかなり大規模な実装です。段階的にリリースすることも検討できます：
- **v0.11.0-alpha**: ジェネリクス + Event Loop（2025年12月）
- **v0.11.0-beta**: Future + Result型（2026年1月）
- **v0.11.0**: async/await完全版（2026年2月）

**A**: 完全に後方互換：
- async/awaitは新しい構文（既存コードに影響なし）
- 既存の同期関数はそのまま動作
- async関数と同期関数を混在可能
- 段階的な移行が可能

### Q5: Result型とtry-catchはどう使い分ける？

**A**: 以下のガイドラインを推奨：

**Result型を使うべき場合**（推奨）：
- ✅ 通常のエラーハンドリング（ファイルが見つからない、ネットワークエラーなど）
- ✅ 予測可能なエラー
- ✅ エラーが戻り値の一部として扱える場合
- ✅ 型安全性を重視する場合
- ✅ エラー伝播を明示的にしたい場合（?オペレーター）

**try-catchを使う場合**（限定的）：
- ⚠️ unwrap/expectによるパニック（開発時のみ）
- ⚠️ 予期しないエラー（プログラムのバグなど）
- ⚠️ レガシーコードとの互換性

**推奨パターン**：
```cb
// ✅ 推奨: Result型で明示的なエラーハンドリング
async Result<User, DbError> fetch_user(int id) {
    if (id <= 0) {
        return Err(DbError{"Invalid ID"});
    }
    // ...
    return Ok(user);
}

// ⚠️ 非推奨: try-catchは最小限に
async User fetch_user_legacy(int id) {
    if (id <= 0) {
        throw DbError{"Invalid ID"};  // 避けるべき
    }
    // ...
    return user;
}

// ✅ Result型の使用例
async void main() {
    Result<User, DbError> result = await fetch_user(1);
    match (result) {
        Ok(user) => println("User: ", user.name),
        Err(error) => println("Error: ", error.message)
    }
}
```

**利点**：
- コンパイル時にエラーハンドリングの漏れを検出
- エラーの型が明確
- エラー伝播が追跡しやすい
- 不要なtry-catchブロックを避けられる

---

## 📝 次のステップ

### ドキュメント作成
1. **async/await仕様書**（本ドキュメントの詳細版）
2. **Event Loop設計書**
3. **Future型のAPI仕様**
4. **Result型のAPI仕様**
5. **ジェネリクス仕様書**（async/await向けの最小限の機能）
6. **チュートリアル**: async/awaitの使い方

### 実装開始（推奨順序）
1. **Phase 0: ジェネリクスの基本実装**（4週間）
   - ジェネリック構造体（`Future<T>`用）
   - ジェネリックEnum（`Result<T, E>`用）
   - 基本的な型パラメータ
2. **Phase 1: Event Loop + タイマー**（2週間）
3. **Phase 2: Future<T>型**（1週間）
4. **Phase 3: Result<T, E> + パターンマッチング**（3週間）
5. **Phase 4: async/await構文**（3週間）
6. **Phase 5: ?オペレーター**（1週間）

**合計見積もり**: 約14週間（3.5ヶ月）

### リリース計画

**v0.11.0の対象**:
- ✅ ジェネリクス（基本機能のみ）
- ✅ Event Loop
- ✅ Future<T>型
- ✅ Result<T, E>型
- ✅ パターンマッチング
- ✅ async/await構文
- ✅ ?オペレーター

**v0.11.1で追加予定**:
- ジェネリクスの型制約
- ジェネリックInterface
- より複雑なパターンマッチング

**v0.12.0で追加予定**:
- スレッドプール
- チャネル
- 型推論

### コミュニティフィードバック
- 本設計書をGitHubで公開
- ユーザーからのフィードバック収集
- 仕様の改善と調整

---

**作成者**: GitHub Copilot  
**レビュアー**: shadowlink0122  
**ステータス**: 設計提案中
