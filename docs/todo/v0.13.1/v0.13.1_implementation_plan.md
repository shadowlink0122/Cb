...
Cb言語では、ジェネリックinterfaceの実装時に`impl<T>`という明示的な型パラメータ宣言は不要です。型パラメータはinterfaceとstructの定義から自動的に推論されます。
...
// ❌ 誤った構文（Rust風だが、Cbではサポートしない）
impl<T> Container<T> for Box<T> {  // エラー: Expected identifier after 'impl'
    ...
}
```

この設計により、構文がシンプルになり、interfaceとstructの定義と一貫性が保たれます。
