...
v0.14.0では、以下の大規模なリファクタリングを実施します：
...
将来的に以下の部分を移行することを検討します（v0.14.0では不要）：
...
v0.14.0から将来のバージョン（v0.15.0以降）への移行期間中は、以下の両方をサポートします：
...
v0.14.0完了後、v0.15.0以降で最適化とコード生成の実装を進めます。
